...
*/				
				...
				console.log(master);
				console.log(master.items.get(0));
				
				var thes = new ThesaurusReader({
					  id:newName + '_PANEL_THESAURUS',
					  lang: master.items.get(0).lang,
				      outputLangs: master.items.get(0).outputLangs, 
				      separator: master.items.get(0).separator,
				      appPath: master.items.get(0).appPath,
				      returnPath: false,
				      returnInspire: true,
				      width: 300, 
				      height:400,
				      layout: 'fit',
				      proxy: master.items.get(0).proxy,
				      handler: function(result){
				      				var target = form.findById(newName);
					...
					handler: function()
	                {
						...
						if (!winthge)
							winthge = new Ext.Window({
							                id: newName + '_WIN',
											title:master.findById( name + '_WIN').title,
							                width:500,
							                height:500,
							                closeAction:'hide',
							                layout:'fit', 
	...
	/*afterRender : Ext.Button.prototype.afterRender.createSequence(function() { 			
		if ( this.dynamic) 
		{		
			...
			this.addListener('onMinusIcon',function(field) 
			{
		    	...
		    	if (!field.clone)
		    	{
					...
					firstClone.extendedTemplate = undefined;
					...
					{
						listOfClones[i].extendedTemplate = firstClone;
					}
					firstClone.manageIcons(firstClone);
			    }
			    else
			    {
			    	var tmpl = field.extendedTemplate;
			    	item.remove();
					fieldset.remove(field, true);
					fieldset.doLayout();
