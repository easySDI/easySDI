...
class SITE_object {
	...
	function saveObject($option){
		...
		if ($rowObject->id == 0)
		{
			...
			$idrow = array();
			$database->setQuery("SELECT CONCAT(ns.prefix,':',a.name) as attribute_isocode, at.isocode as type_isocode FROM #__sdi_profile p, #__sdi_objecttype ot, #__sdi_relation rel, #__sdi_list_attributetype as at, #__sdi_attribute a RIGHT OUTER JOIN #__sdi_namespace ns ON ns.id=a.namespace_id WHERE p.id=ot.profile_id AND rel.id=p.metadataid AND a.id=rel.attributechild_id AND at.id=a.attributetype_id AND ot.id=".$rowObject->objecttype_id);
			$idrow = array_merge( $idrow, $database->loadObjectList() );
			
			// Récupérer la classe racine
			$root = array();
			$database->setQuery("SELECT CONCAT(ns.prefix,':',c.name) FROM #__sdi_profile p, #__sdi_objecttype ot, #__sdi_class c RIGHT OUTER JOIN #__sdi_namespace ns ON ns.id=c.namespace_id WHERE c.id=p.class_id AND p.id=ot.profile_id AND ot.id=".$rowObject->objecttype_id);
