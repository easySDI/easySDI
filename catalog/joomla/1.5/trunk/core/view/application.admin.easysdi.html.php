	...
	<form action="index.php" method="POST" name="adminForm">
		...
		<thead>
			<tr>
				...
				<?php echo JHTML::_('grid.order',  $rows, 'filesave.png', 'saveOrderApplication' ); ?></th>
				...
				<th class='title'><?php echo JHTML::_('grid.sort',   JText::_("CATALOG_APPLICATION_WINDOWNAME"), 'windowname', @$filter_order_Dir, @$filter_order); ?></th>
				<th class='title'><?php echo JHTML::_('grid.sort',   JText::_("CATALOG_APPLICATION_URL"), 'url', @$filter_order_Dir, @$filter_order); ?></th>
			...
			<tr>
				...
				<td width="100px" align="right">
					...
					if ($filter_order=="ordering" and $filter_order_Dir=="asc"){
						if ($disabled){
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderupApplication', '', false ); ?>
				             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderdownApplication', '', false ); ?>
		            <?php
						}
						else {
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderupApplication', 'Move Up', isset($rows[$i-1]) ); ?>
				             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderdownApplication', 'Move Down', isset($rows[$i+1]) ); ?>
					<?php
						}		
					}
					else{ 
						if ($disabled){
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderdownApplication', '', false ); ?>
				             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderupApplication', '', false ); ?>
		            <?php
						}
						else {
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderdownApplication', 'Move Down', isset($rows[$i-1]) ); ?>
		 		             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderupApplication', 'Move Up', isset($rows[$i+1]) ); ?>
					<?php
						}
					}?>
...
<?php
		...
		<td colspan="9"><?php echo $page->getListFooter(); ?></td>
	  	...
	  	<input type="hidden" name="task" value="listApplication" />
