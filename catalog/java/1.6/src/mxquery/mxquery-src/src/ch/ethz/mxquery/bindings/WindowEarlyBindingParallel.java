...
public final class WindowEarlyBindingParallel extends WindowTokenIterator {
	...
	public WindowEarlyBindingParallel(WindowBuffer mat, int id, int startPosition, WindowVariable[] startVars, VariableHolder[] startVarHolders, XDMIterator endExpr, WindowVariable[] endVars, VariableHolder[] endVarHolders) {
	 ...
	 */
	private boolean checkEndExpr(int startNode, int endNode)  throws MXQueryException {
		boolean value=false;
	 ...
	 * @throws MXQueryException
	 */
	protected void assignVars(WindowVariable[] vars, VariableHolder[] varHolders, int nodeId)throws MXQueryException {
		if(vars !=null){
			for(int i =0; i < vars.length; i++){
				...
				case WindowVariable.WINDOW_VAR_POSITION:
					varHolders[i].setIter(new TokenIterator(context, new LongToken(Type.INT, null, nodeId - startNodeId + 1),null, loc));
					//varHolders[i].setIter(new TokenIterator(context, new LongToken(Type.INTEGER, null, nodeId - startNodeId + 1), null));
					break;
				}
			}
		}
	...
	public XDMIterator nextItem() throws MXQueryException {
		...
		if(checkItem(currentNodeId)){
			return mat.getNewWindowIterator(currentNodeId +1 , currentNodeId +1);
		}else{
			return mat.getEmptySequence();
		}
	}
	...
	private WindowIterator getNewWindowIteratorWithNodes(int startNode, int endNode) {
		if(!isWindowInUse() && startNode == 0 && endNode == END_OF_STREAM_NODE ){
			...
			return mat.getNewWindowIteratorWithNodeIds(startNodeId + startNode, min(this.endNode, startNodeId + endNode));
		}
	}
	...
	protected XDMIterator copy(Context context, XDMIterator[] subIters, Vector nestedPredCtxStack) throws MXQueryException {
		throw new RuntimeException("copy of WindowEarlyBinding not yet implemented!");
	}
