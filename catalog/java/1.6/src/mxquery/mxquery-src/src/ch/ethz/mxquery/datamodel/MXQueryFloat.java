...
public class MXQueryFloat implements MXQueryNumber {
	...
	if (s.equals(VALUE_NAN)) {
	    val = Float.NaN;
	    return;
	} else if (s.equals(VALUE_POS_INFINITY)) {
	    val = Float.POSITIVE_INFINITY;
	    return;
	} else if (s.equals(VALUE_NEG_INFINITY)) {
	...
	// if (isNaN() || d.isNaN() || isNegativeInfinity() ||
	// d.isNegativeInfinity() ||
	// isPositiveInfinity() || d.isPositiveInfinity())
	// throw new
	// DynamicException(ErrorCodes.F0003_OVERFLOW_UNDERFLOW_NUMERIC,
	// "Invalid input for idiv");
	if (d.isNaN() || isNaN() || isNegativeInfinity()
		|| isPositiveInfinity()) {
	    throw new DynamicException(
		    ErrorCodes.F0003_OVERFLOW_UNDERFLOW_NUMERIC,
		    "Invalid input for idiv", null);
	}
	...
	try {
	    ...
	    throw new MXQueryException(ErrorCodes.F0002_DIVISION_BY_ZERO,
    ...
    /*
     * -1, 0 or 1 as this MXQueryDouble is numerically less than, equal to, or
     * greater than parameter d
     */
	...
	if (isPositiveInfinity() || isNegativeInfinity() || isNaN())
	    throw new DynamicException(ErrorCodes.F0005_INVALID_LEXICAL_VALUE,
		    "Not an integer value", null);
	...
	if (res < tmp)
	    throw new DynamicException(ErrorCodes.A0002_EC_NOT_SUPPORTED,
		    "Big Integer values are not supported.", null);
	...
	BigInteger intv = iv1.round(MathContext.DECIMAL32).toBigInteger();
	...
	// double val =
	// Math.round(inputValue.getValue())-inputValue.getIntValue();

	BigDecimal eps = iv.divide(new BigDecimal(scale / 1000),
		BigDecimal.ROUND_HALF_EVEN).abs();
     ...
     *                for the differences between the Java and XPath rules.
     * @param isNegativeZero
     * @return the value converted to a string, according to the XPath casting
     *         rules.
     */
    public static String toString(float val, String javaStrValue,
	...
	if ((val > 0 && (val < (1.0e-6) - 5e-10 || val >= 1.0e+6))
		...
		if (matcher.matches()) {
			...
			sb.append("E"
				+ (matcher.group(2).length() + matcher.group(3)
					.length()));
			return sb.toString();
		    } else {
	...
	int e = s.indexOf('E');
	if (e < 0) {
	    // For some reason, Double.toString() in Java can return strings
	    // such as "0.0040"
    ...
    static java.util.regex.Pattern nonExponentialPattern = java.util.regex.Pattern
	    .compile("(-?[0-9])([0-9]+?)(0*)\\.([0-9]*)");
