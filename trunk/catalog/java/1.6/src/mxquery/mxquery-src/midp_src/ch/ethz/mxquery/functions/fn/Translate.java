...
public class Translate extends TokenBasedIterator {
	...
	protected void init() throws MXQueryException {
		...
		for (int i=0; i<sMap.length(); i++) {
			...
			if (sTrans.length() > transPos) {
				...
				int tMapChar = sTrans.charAt(transPos);
	...
	private String translate(String input) {
		...
		for (int i=0; i<input.length(); i++) {
			Integer character = new Integer(input.charAt(i));
			if (!remove.contains(character)) {
				if (map.containsKey(character)) {
					result.append((char)((Integer)map.get(character)).intValue());
				} else {
					result.append((char)character.intValue());
				}
			}
