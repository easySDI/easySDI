...
class HTML_metadata {
	...
	function editMetadata($product_id, $root, $metadata_id, $xpathResults, $option)
	{
		...
		?>
<!-- Pour permettre le retour ï¿½ la liste des produits depuis la toolbar Joomla -->
<div id="page">
<form action="index.php" method="post" name="adminForm" id="adminForm"
	...
	value="" /></form>
		...
		<?php
		$this->javascript .="
				var domNode = Ext.DomQuery.selectNode('div#page')
	...
	function buildTree($database, $parent, $parentFieldset, $parentName, $xpathResults, $scope, $queryPath, $currentIsocode, $partner_id, $option)
	{
		...
		foreach($rowAttributeChilds as $child)
		{
			...
			// Ajout d'une occurence de crï¿½ation si l'attribut est obligatoire
	 ...
	 foreach($rowListClass as $child)
	 {
		...
		foreach($rowLocText as $child)
		{
			...
			// Ajout d'une occurence de crï¿½ation si la classe est obligatoire
			// et qu'il n'y a aucune occurence de celle-ci dans le XML
		...
		// Rï¿½cupï¿½ration des classes enfants du noeud
		...
		{
			// Compte du nombre d'occurence de ce noeud (Multiplicitï¿½)
...
/* 
 * ï¿½ mettre en place ou supprimer dans le futur
 * [23.1.0.2009] Stï¿½phanie Gilliï¿½ron
 * 
	...
	function constructLocfreetext($pos, $parentFieldset, $fieldsetId, $id, $label, $lang, $min, $max, $clone, $master)
	{
		...
		foreach($langages as $lang)
		{
			...
			$this->javascript .="
				fieldset".$fieldsetId.".add(createTextArea('".$LocLangName."', '".html_Metadata::cleanText(JText::_($lang->translation))."', true, false, null, '1', '1', '".$nodeValue."'));
				// Crï¿½ation du champ cachï¿½ (qui conservera l'index) liï¿½ au bloc de champs multiple
...
<table class="adminlist">
	<thead>
		<tr>
			...
			<th class='title' width="10px"><input type="checkbox" name="toggle"
	...
	foreach ($rows as $row)
	{
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td width="10px"><input type="checkbox" id="cb<?php echo $i;?>"
			...
			}?> <input type="text" id="or<?php echo $i;?>" name="order[]"
				size="5" <?php echo $disabled; ?>
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataTabs" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"> <input type="hidden"
	name="filter_order_Dir" value="<?php echo $filter_order_Dir; ?>" /> <input
	type="hidden" name="filter_order" value="<?php echo $filter_order; ?>" />
</form>
	<?php
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><input size="50" type="text" name="text"
...
<input type="hidden" name="order" value="0" /> <input type="hidden"
	...
	name="id" value="<?php echo $row->id?>" /> <input type="hidden"
...
<table>
	<tr>
		<td align="left" width="100%"><?php echo JText::_( 'Filter' ); ?>: <input
			type="text" name="searchStd" id="searchStd"
			value="<?php echo $search;?>" class="text_area"
			onchange="document.adminForm.submit();" />
		<button onclick="this.form.submit();"><?php echo JText::_( 'Go' ); ?></button>
		<button
			onclick="document.getElementById('searchStd').value='';this.form.submit();"><?php echo JText::_( 'Reset' ); ?></button>
		</td>
	</tr>
</table>
<table width="100%">
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataStandardClasses" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"> <input type="hidden"
	name="filter_order_Dir" value="<?php echo $filter_order_Dir; ?>" /> <input
	type="hidden" name="filter_order" value="<?php echo $filter_order; ?>" />
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataStandard" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"></form>
	<?php
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><select name="is_global">
			<option value="1" <?php if($row->is_global == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_global == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataExt" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"></form>
	<?php
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><input size="50" type="text" name="translation"
			value="<?php echo $row->translation?>"></td>
	</tr>
	<tr>
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataFreetext" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"></form>
	<?php
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><input size="50" type="text" name="description"
			value="<?php echo $row->description?>"></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_LOCFREETEXT_LANG"); ?></td>
		<td><input size="50" type="text" name="lang"
			value="<?php echo $row->lang?>"></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_LOCFREETEXT_PARTNER_ID"); ?></td>
		<td><?php echo JHTML::_("select.genericlist",$partners, 'partner_id', 'size="1" class="inputbox"', 'value', 'text', $row->partner_id ); ?></td>
	...
	<tr>
		...
		<td><input size="50" type="text" name="default_value"
			value="<?php echo $row->default_value?>"></td>
	</tr>
...
<table class="adminlist">
	<thead>
		<tr>
			...
			<th width="10px" class='title'><input type="checkbox" name="toggle"
				value="" onclick="checkAll(<?php echo count($rows); ?>);" /></th>
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataClass" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"> <input type="hidden"
	name="filter_order_Dir" value="<?php echo $filter_order_Dir; ?>" /> <input
	type="hidden" name="filter_order" value="<?php echo $filter_order; ?>" />
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><select name="has_xlinkTitle">
			<option value="1"
			<?php if($row->has_xlinkTitle == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0"
			<?php if($row->has_xlinkTitle == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_CLASS_TRANSLATION"); ?></td>
		<td><input size="100" type="text" name="translation"
			value="<?php echo $row->translation?>"></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><select name="is_final">
			<option value="1" <?php if($row->is_final == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_final == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><input size="50" type="text" name="iso_key"
			value="<?php echo $row->iso_key?>"></td>
	</tr>
	<tr>
		...
		<td><select name="type" id="type" onChange="selectTypeParameters()">
			...
			<?php if($row->type == "freetext") echo "selected"; ?>><?php echo JText::_("EASYSDI_FREETEXT"); ?></option>
			<option value="locfreetext"
			<?php if($row->type == "locfreetext") echo "selected"; ?>><?php echo JText::_("EASYSDI_LOCFREETEXT"); ?></option>
			<option value="ext" <?php if($row->type == "ext") echo "selected"; ?>><?php echo JText::_("EASYSDI_EXT"); ?></option>
			<option value="class"
			<?php if($row->type == "class") echo "selected"; ?>><?php echo JText::_("EASYSDI_CLASS"); ?></option>
			<option value="list"
			<?php if($row->type == "list") echo "selected"; ?>><?php echo JText::_("EASYSDI_LIST"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_CLASS_IS_RELATION"); ?></td>
		<td><select name="is_relation">
			<option value="1"
			<?php if($row->is_relation == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0"
			<?php if($row->is_relation == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_CLASS_LOWERBOUND"); ?></td>
		<td><input size="3" type="text" name="lowerbound"
			value="<?php echo $row->lowerbound?>"></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_CLASS_UPPERBOUND"); ?></td>
		<td><input size="3" type="text" name="upperbound"
			value="<?php echo $row->upperbound?>"></td>
	</tr>
	<tr>
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><select name="is_system">
			<option value="1" <?php if($row->is_system == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_system == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_FREETEXT_IS_DATE"); ?></td>
		<td><select name="is_date">
			<option value="1" <?php if($row->is_date == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_date == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_FREETEXT_IS_DATETIME"); ?></td>
		<td><select name="is_datetime">
			<option value="1"
			<?php if($row->is_datetime == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0"
			<?php if($row->is_datetime == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_FREETEXT_IS_NUMBER"); ?></td>
		<td><select name="is_number">
			<option value="1" <?php if($row->is_number == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_number == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_FREETEXT_IS_INTEGER"); ?></td>
		<td><select name="is_integer">
			<option value="1" <?php if($row->is_integer == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_integer == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_FREETEXT_IS_SHORTTEXT"); ?></td>
		<td><select name="is_shorttext">
			<option value="1" <?php if($row->is_shorttext == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0" <?php if($row->is_shorttext == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<!-- 
	<tr>
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><input size="50" type="text" name="code_key"
			value="<?php echo $rowMDList->code_key?>"></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_LIST_ISO_CODE"); ?></td>
		<td><input size="50" type="text" name="key"
			value="<?php echo $rowMDList->key?>"></td>
	</tr>
	<!-- 						
	<tr>
	...
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_TRANSLATION"); ?></td>
		<td><input size="50" type="text" name="translation"
			value="<?php echo $rowMDList->translation?>"></td>
	</tr>
	<tr>
...
<table border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td><input size="50" type="text" name="name"
			value="<?php echo $rowMDList->name?>"></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_TRANSLATION"); ?></td>
		<td><input size="50" type="text" name="translation"
	...
	<tr>
		...
		<td><select name="multiple">
			<option value="1"
			<?php if($rowMDList->multiple == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0"
			<?php if($rowMDList->multiple == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_ISOKEY"); ?></td>
		<td><input size="50" type="text" name="iso_key"
			value="<?php echo $rowMDList->iso_key?>"></td>
	</tr>
	<tr>
		<td><?php echo JText::_("EASYSDI_METADATA_LIST_CODEVALUE"); ?></td>
		<td><select name="codeValue">
			<option value="1"
			<?php if($rowMDList->codeValue == 1) echo "selected"; ?>><?php echo JText::_("EASYSDI_TRUE"); ?></option>
			<option value="0"
			<?php if($rowMDList->codeValue == 0) echo "selected"; ?>><?php echo JText::_("EASYSDI_FALSE"); ?></option>
		</select></td>
	</tr>
</table>
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="id" value="<?php echo $rowMDList->id?>" /> <input
	type="hidden" name="task" value="" /></form>
			<?php
...
<input type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="listMetadataList" /> <input
	type="hidden" name="list_id" value="<?php echo $list_id; ?>" /> <input
	type="hidden" name="boxchecked" value="0" /> <input type="hidden"
	name="hidemainmenu" value="0"></form>
	<?php
