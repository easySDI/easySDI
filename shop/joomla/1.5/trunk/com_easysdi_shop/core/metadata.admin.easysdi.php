...
class ADMIN_metadata {
	...
	function buildXMLTree($parent, $parentFieldset, $parentName, $doc, $XMLDoc, $xmlParent, $queryPath, $currentIsocode, $option)
	{
		...
		foreach($rowClassChilds as $child)
		{
			...
			for ($pos=2; $pos<=$index; $pos++)
			{
				...
				ADMIN_metadata::buildXMLTree($child->classes_to_id, $child->classes_to_id, $name, &$doc, &$XMLDoc, $XMLNode, $queryPath, $nextIsocode, $option);
			}
		}
	...
	function saveMetadata($option)
	{
		...
		ADMIN_metadata::buildXMLTree('4001', '4001', "//gmd:MD_Metadata", $doc, $XMLDoc, $XMLNode, $path, 'gmd:MD_Metadata', $option);
