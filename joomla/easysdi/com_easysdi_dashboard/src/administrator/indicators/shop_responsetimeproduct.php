...
class sdiIndicatorShop_responsetimeproduct extends sdiIndicator {
    ...
    protected function _getData($organism, $timestart, $timeend, $limit = 0) {
        ...
        //1h-24h
        $query = $db->getQuery(true);
        $query->select($query->concatenate(array('\'1h-24h (\'', $query->castAsChar('count(odif.id)'), '\')\'')) . ' as timerange, count(odif.id) as count')
            ->from($db->quoteName('#__sdi_order', 'o'))
            ->join('INNER', $db->quoteName('#__sdi_order_diffusion', 'odif') . ' ON (' . $db->quoteName('o.id') . ' = ' . $db->quoteName('odif.order_id') . ')')
            ->join('INNER', $db->quoteName('#__sdi_diffusion', 'dif') . ' ON (' . $db->quoteName('odif.diffusion_id') . ' = ' . $db->quoteName('dif.id') . ')')
            ->join('INNER', $db->quoteName('#__sdi_version', 'v') . ' ON (' . $db->quoteName('dif.version_id') . ' = ' . $db->quoteName('v.id') . ')')
        ...
        if ($db->name == 'mysqli') {
            $query
                ->where('UNIX_TIMESTAMP(' . $db->quoteName('odif.completed') . ') -  UNIX_TIMESTAMP(' . $db->quoteName('o.sent') . ') <= 86400')
        ...
        $db->setQuery($query, 0, $limit);
        $res_1h_1d = $db->loadRowList();

        //24h-48h
        $query = $db->getQuery(true);
        $query->select($query->concatenate(array('\'24h-48h (\'', $query->castAsChar('count(odif.id)'), '\')\'')) . ' as timerange, count(odif.id) as count')
                ->from($db->quoteName('#__sdi_order', 'o'))
                ->join('INNER', $db->quoteName('#__sdi_order_diffusion', 'odif') . ' ON (' . $db->quoteName('o.id') . ' = ' . $db->quoteName('odif.order_id') . ')')
                ->join('INNER', $db->quoteName('#__sdi_diffusion', 'dif') . ' ON (' . $db->quoteName('odif.diffusion_id') . ' = ' . $db->quoteName('dif.id') . ')')
                ->join('INNER', $db->quoteName('#__sdi_version', 'v') . ' ON (' . $db->quoteName('dif.version_id') . ' = ' . $db->quoteName('v.id') . ')')
        ...
        $return = new stdClass();
        $return->data = array_merge($res_lt1h, $res_1h_1d, $res_1d_2d, $res_mt2d);
