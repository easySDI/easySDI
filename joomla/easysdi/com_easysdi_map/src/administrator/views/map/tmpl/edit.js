...
function clearMaps() {
    jQuery("#jform_default_backgroud_layer_new").empty();
    jQuery("#jform_default_backgroud_layer_new").trigger("liszt:updated");
}

function getMaps() {
    clearMaps();

    selectedservice = jQuery('#jform_background').find(":selected").val();
    if (!selectedservice)
        return;
    ...
    if (layers) {
        ...
        if (jQuery("#jform_onloadlayername").val()) {
            jQuery("#jform_default_backgroud_layer_new option[value='" + jQuery("#jform_onloadlayername").val() + "']").attr("selected", "selected");
            jQuery("#jform_default_backgroud_layer_new").trigger("liszt:updated");
        }
    } else {
        request = false;
        ...
        var query = "index.php?option=com_easysdi_map&task=getMaps&group=" + selectedservice;
...
function addLayerOption(id, value) {
    jQuery("#jform_default_backgroud_layer_new").append('<option value="' + id + '">' + value + '</option>');
    jQuery("#jform_default_backgroud_layer_new").trigger("liszt:updated");
}
...
function setLayers() {
    if (request.readyState == 4) {
        ...
        JSONobject.forEach(function(element) {
            ...
            if (element) {
                addLayerOption(element.id, element.name);
            }
        }, this);


        if (ok) {
            if (jQuery("#jform_default_backgroud_layer").val()) {
                jQuery("#jform_default_backgroud_layer_new option[value='" + jQuery("#jform_default_backgroud_layer").val() + "']").attr("selected", "selected");
                jQuery("#jform_default_backgroud_layer_new").trigger("liszt:updated");
            }
...
function setServiceConnector() {
    ...
    var service = jQuery('#jform_background').find(":selected").val();
