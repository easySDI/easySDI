...
$saveOrder = $listOrder == 'llg.ordering';
if ($saveOrder) {
...
<form action="<?php echo JRoute::_('index.php?option=com_easysdi_map&view=layers'); ?>" method="post" name="adminForm" id="adminForm">
    <?php if (!empty($this->sidebar)): ?>
        ...
        this.form.submit();"><i class="icon-remove"></i></button>
                    ...
                    <tr>
                        <?php if (isset($this->items[0]->groupordering)): ?>
                            <th width="1%" class="nowrap center hidden-phone">
                                <?php echo JHtml::_('grid.sort', '<i class="icon-menu-2"></i>', 'llg.ordering', $listDirn, $listOrder, null, 'asc', 'JGRID_HEADING_ORDERING'); ?>
                            </th>
                        <?php else : ?>
                            <th width="1%" class="nowrap center hidden-phone">
                        ...
                        <?php endif; ?>
                        <th class='left'>
                            <?php echo JHtml::_('grid.sort', 'COM_EASYSDI_MAP_FORM_LBL_LAYER_NAME', 'a.name', $listDirn, $listOrder); ?>
                        </th>
                        <th class='left'>
			...
			$canEdit	= $user->authorise('core.edit',	'com_easysdi_map.layer.'. $item->id);
                        $canCheckin     = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
                        $canEditOwn     = $user->authorise('core.edit.own',   'com_easysdi_map.layer.' . $item->id) && $item->created_by == $userId;
                        ...
                        <tr class="row<?php echo $i % 2; ?>" >
                            ...
                            <td class="nowrap has-context">
                                ...
                                <div class="pull-left">
                                    ...
                                    JHtml::_('dropdown.edit', $item->id, 'layer.');
                                    JHtml::_('dropdown.divider');
                                    if ($item->state) :
                                        JHtml::_('dropdown.unpublish', 'cb' . $i, 'layers.');
                                    else :
                                        JHtml::_('dropdown.publish', 'cb' . $i, 'layers.');
                                    endif;
                                    JHtml::_('dropdown.divider');

                                    if ($archived) :
                                        JHtml::_('dropdown.unarchive', 'cb' . $i, 'layers.');
                                    else :
                                        JHtml::_('dropdown.archive', 'cb' . $i, 'layers.');
                                    endif;

                                    if ($item->checked_out) :
                                        JHtml::_('dropdown.checkin', 'cb' . $i, 'layers.');
                                    endif;

                                    if ($trashed) :
                                        JHtml::_('dropdown.untrash', 'cb' . $i, 'layers.');
                                    else :
                                        JHtml::_('dropdown.trash', 'cb' . $i, 'layers.');
                                    endif;
