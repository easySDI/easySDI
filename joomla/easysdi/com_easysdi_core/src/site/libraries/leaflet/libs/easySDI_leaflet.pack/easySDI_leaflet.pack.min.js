...
jQuery.each(contextMapData.groups,function(g,group){var overlay=(group.isbackground!='1');if(isset(group.layers)){jQuery.each(group.layers,function(l,layer){var show=(layer.isdefaultvisible=='1');addLayer(layer,overlay,show,group.name);});}});var geoJsonDataObj=obj.siblings('.addGeoJson');if(geoJsonDataObj.length>0){var ld=L.geoJson(JSON.parse(geoJsonDataObj.html()),{style:{className:'addedGeoJsonLayer'}});overlays['textareaGeoJson']=ld;ld.addTo(map);geoJsonDataObj.remove();}
