...
abstract class ModLanguagesHelper {
    ...
    private static function getQueryString() {
        ...
        $confsef = JFactory::getConfig()->get('sef');
        if ($confsef == 0)
            return "";
        ...
        foreach ($data as $k => $v) {
            if ($k != 'language' && $k != 'lang' && $k != 'Itemid' && $k != 'format' && $k != 'option' && $k != 'layout') {
                if ($k == 'view' || $k == 'id') {
                    if ($data['option'] == 'com_easysdi_catalog') {
                        $query .= "&" . $k . "=" . $v;
                    }
                    continue;
                }
                ...
                if ($k == 'jform') {
                    ...
                    foreach ($v as $key => $values) {
                        if (is_array($values)) {
                            foreach ($values as $minkey => $minvalue) {
                                $i = is_int($minkey) ? '' : $minkey;
                                $param .= 'jform[' . $key . '][' . $i . ']=' . $minvalue . '&';                                
                            }
                        } else {
                            $param .= 'jform[' . $key . ']=' .$values . '&';                            
                        }
                    }
                    $param = str_replace("[", "%5B", $param);
