...
/**
* A plugin which displays the panel title when it is collapsed if oriented vertically.
* Set this to the plugin option when setting up a panel.
*/
...
/**
 * Standard layout class for the web map module. Places each UI element in the correct place.
 */
EasySDI_Map.RwgLayout = Ext.extend(EasySDI_Map.LayoutBase, {
  constructor: function(config){
    ...
    if (componentParams.authorisedTo.DATA_PRECISION && componentDisplayOption.DataPrecisionEnable) {
        ...
        items: new Ext.TabPanel({
	  	...
	  	// data precision panel not authorised so just display layers	  	
...
//      }),
    ...
    //Add the keyMap on the zoomToScale field after layout rendering that the this.mapPanel.zoomToScaleField.id was initialized
    //this.mapPanel.zoomToScaleField can be null if the toolbar is not displayed
  ...
  /**
   * Construct the panels that are required on startup of the page
   */
  _initPanels: function() {
    this.mapPanel = new EasySDI_Map.MapPanel({
      ...
      overviewWidth: 120,
      overviewHeight: 170});     
    //this.searchPanel = new EasySDI_Map.SearchPanel({region: "north", border: false});
    this.searchPanel = new EasySDI_Map[extensionClasses.SearchPanelClassName](
    		{
    			region: "north", 
    			border: false}
    		);
    this.layerTree = new EasySDI_Map.LayerTree({map : this.mapPanel});
    ...
    this.layerPanel = new EasySDI_Map.LayerPanel({
      ...
      tree : this.layerTree,
      // Occupy the west region of the entire viewport if not in the tab panel
      region: componentParams.authorisedTo.PRECISE_DATA && componentDisplayOption.DataPrecisionEnable? null : "west",
      width: componentDisplayOption.DataPrecisionEnable ? 300 : 200
    });
  ...
  /**
   * showFilterPanel specific for this layout it replaces the legend.
   */
  showFilterPanel : function(innerSearchBar) {
    ...
    if (!this.filterPanel) {
      ...
      //this.filterPanel = new EasySDI_Map.RwgFilterPanel({mapPanel: this.mapPanel, innerSearchBar: innerSearchBar});
      //this.filterPanel = new EasySDI_Map.FilterPanel({mapPanel: this.mapPanel, innerSearchBar: innerSearchBar});
      this.filterPanel = new EasySDI_Map[extensionClasses.FilterPanelClassName]({mapPanel: this.mapPanel, innerSearchBar: innerSearchBar});
      
      this.legendOrFilterPanel.add(this.filterPanel);
      this.searchPanel.registerTrigger('filterPanel.setMode',
          this.filterPanel.SetMode.createDelegate(this.filterPanel));
      this.searchPanel.registerTrigger('filterPanel.updateFilter',
          this.filterPanel.updateFilter.createDelegate(this.filterPanel));
      ...
      this.filterPanel.registerTrigger('hideFilterPanel', this.hideFilterPanel.createDelegate(this));          
    	...
    	// Ensure the old filter panel links to the current search bar.
    	this.filterPanel.innerSearchBar = innerSearchBar;
      // Drawing and Selection layers visible if the user can access this tab
