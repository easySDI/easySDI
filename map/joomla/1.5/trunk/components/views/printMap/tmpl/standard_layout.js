 ...
 *
 * The purpose of this page is to give a way to print a view of the map with only :
 * - base map
 * - visible layers
 * - all annotations
 * - graphic scale
 * - no additional panel, just the MapPanel
 * This replace the functionality of saving the map as an image (jpg, png).
 */
EasySDI_Map.printMapLayout = Ext.extend(Ext.Viewport, {
  constructor: function(config) 
  {
  	//this contains all the map elements description (layers, features, styles)
	...
	var options = {
			...
			//restrictedExtent: new OpenLayers.Bounds(this.mapDescriber.extent.left,this.mapDescriber.extent.bottom,this.mapDescriber.extent.right,this.mapDescriber.extent.top),
 ...
 */
	...
	Ext.each(SData.baseLayers, this._addBaseLayer, this);
  	this.map.zoomToExtent(new OpenLayers.Bounds(this.mapDescriber.extent.left,this.mapDescriber.extent.bottom,this.mapDescriber.extent.right,this.mapDescriber.extent.top));
	Ext.each(SData.overlayLayers, this._addLayers, this);
	Ext.each(annotationStyle, this._addAnnotations, this);
	
	//Define the mapPanel
	//Give it the size of the mapPanel displaying in the rwgLayout (main map component)
	...
	*/
	_addBaseLayer : function (layer)
	{
		if (this.mapDescriber.basemap == layer.layers)
		{
			...
			var l = new OpenLayers.Layer.WMS(layer.name,layer.url,
	        WMSoptions, {
			...
			if ( layerToAdd.name ==  layer.layers)
			{
				...
				switch (layer.url_type.toUpperCase())
			    {
			      ...
			      case 'WMS':
			        ...
			        l = new OpenLayers.Layer.WMS(layer.name,layer.url,
			          WMSoptions, {          	
			        	//maxExtent:new OpenLayers.Bounds(layer.maxExtent.left,layer.maxExtent.bottom,layer.maxExtent.right,layer.maxExtent.top),
			          	//minScale: layer.minScale,
	          			//maxScale: layer.maxScale,              
			          	//units: layer.units,
			          	isBaseLayer: false,
			        ...
			        //A style has been locally defined by the user : apply it to the layer
			        if(this.mapDescriber.style && this.mapDescriber.style[layer.id])
				    {
				    	//var sld = this._getSLD(layer.layers,this.mapDescriber.style[layer.id].fillColor,this.mapDescriber.style[layer.id].opacity,this.mapDescriber.style[layer.id].strokeColor );
				    	sld = EasySDI_Map.StyledLayerDescriptor.getSLDHeader();
				    	sld += EasySDI_Map.StyledLayerDescriptor.getSLD(layer.layers,this.mapDescriber.style[layer.id].fillColor,this.mapDescriber.style[layer.id].opacity,this.mapDescriber.style[layer.id].strokeColor);
				    	sld += EasySDI_Map.StyledLayerDescriptor.getSLDFooter(); 
				    	l.mergeNewParams({SLD_BODY: sld});
				    };
			      ...
			      case 'WFS':
			        l = new OpenLayers.Layer.WFS(layer.name,
			          componentParams.proxyURL.asString+"&url="+layer.url,
			        ...
			        if(this.mapDescriber.style[layer.id])
				    {
						var style = new OpenLayers.Style({
						 ...
						 l.styleMap = new OpenLayers.StyleMap({"default" : style});
						 l.refresh();
					}
					break;
				}
		...
		Ext.each(this.mapDescriber.layers, function(layerToAdd)
		{
			if ( layerToAdd.name ==  layer.text)
			{
				...
				var template = {
		     	...
		     	//Create the vector layer that will contain annotations
		     	var style = {"default" : new OpenLayers.Style(template)};
				var vectors = new OpenLayers.Layer.Vector(layer.text,{isBaseLayer: false,transparent: "true",styleMap: new OpenLayers.StyleMap(style)});
				
				//Add features (=annotations) to the layer.
				//The features are send to this view as a string : 
				//{POLYGON((x y,x y,x y,x y));POINT(x y);LINESTRING(x y,x y,x y)}
				//Each feature definition needs to be extracted from this string and  
