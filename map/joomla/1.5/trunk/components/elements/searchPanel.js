...
EasySDI_Map.InnerSearchBar = Ext.extend(Ext.Panel, {
  ...
  initComponent: function() {
    ...
    // Because there are events on some of the stores (and they are 'remote') these stores can
    // not be shared between the search bars. Their creation has therefore been moved from the
    ...
    // The simpleFor stores are all remote, so are created for each bar individually. They also have
    // an event on them (to populate the combobox automatically if there is only one entry in the
    // store - if the same store is shared across bars, then all will be set together).
        ...
        if (simpleSearch.dropDownFeatureType === '') {
            ...
            this.simpleForStores.push(emptySimpleStore);
             ...
             var storeOptions = {
                ...
                fields: [
                    {name: simpleSearch.dropDownIdAttr || 'fid', type: 'string'},
                    {name: simpleSearch.dropDownDisplayAttr, type: 'string'}
                ],
    ...
    // This assumes that the default has a valid WFS entry: if this is not the case, then the look up
...
//        triggerAction: 'all',
    ...
    // The inplacestore store is remote, so is created for each bar individually. It also has
    // an event on it (to populate the combobox automatically if there is only one entry in the
    // store - if the same store is shared across bars, then all will be set together).
    if (SData.localisationLayers.length > 0) {
        var storeObject = {
          fields: [
                   {name: 'ipa_fullid', type: 'string'},
                   {name: 'ipa_display_name', type: 'string'}
                  ],
          wfslist: []
        };
        Ext.each(SData.localisationLayers, function(loc) {
          var parts = loc.feature_type_name.split(":");
          // Note that in the fieldlist the "fid" is a generated feature id comprising the
          // feature type and the id - the id is not available as a distinct entry in the record
        ...
        this.inplacestore = new Ext.data.SimpleStore({
            ...
            data: [
              ['0', EasySDI_Map.lang.getLocal('SP_ERROR_NO_LOCALISATION')]]
        });
      }
...
//        forceSelection: true,
    ...
    this.inplaceAutocomplete.store.on('load', function () {
    	// The load event actually gets fired once for each of the WFS calls in the multiWFS: ie if there are 
    	// 3 different calls to WFS services for this WFSmultistore, then the load event will be called once
   ...
   this.simpleGoButton = new Ext.Toolbar.Button({
	...
	//If the search layer is not defined, disable the simpleGoButton to avoid error generation and non sense request
	...
	//If the search layer is not defined, disable the advancedGoButton to avoid error generation and non sense request
	if(!SData.searchLayer)
	{
    ...
    if  (componentDisplayOption.SimpleSearchEnable  && componentDisplayOption.AdvancedSearchEnable  )
    {
    	   defConfig = {
		      ...
		      items: [
		      {
		        ...
		        items: [
		          (this.barIdx+1).toString(),          
		          {xtype: 'tbseparator'},
		          {
							    xtype: 'box',
							    autoEl: {tag: 'img', src:'templates/easysdi_map/icons/silk/find.png'}
							},
		          EasySDI_Map.lang.getLocal('SP_SEARCH'),
		          this.simpleMethod,
		          EasySDI_Map.lang.getLocal('SP_FOR'),
		          this.simpleFor,
		          EasySDI_Map.lang.getLocal('SP_IN_PLACE'),
		          this.inplaceAutocomplete,
		          ' ',
		          this.simpleGoButton,
		          spacer,
		          ...
		          EasySDI_Map.lang.getLocal('SP_BUILD_ADV_SEARCH'),
		          this.advancedMethod,
		          ' ',
		          advancedGoButton,
		          loadButton,
    ...
    // Keep track of other stuff related to the bar, so we can delete them when destroyed.
    this.featureGrid=null;
    this.distinctGrids=[]; // TODO - handle distincts in a proper configurable manner
    ...
    this.on('resize', function(cmp, adjWidth, adjHeight, rawWidth, rawHeight) {
    	...
    	if (size.width<1000) {
    			// Running on a very small screen, so squash the search bar as much as possible
          ...
          this.addButton.setVisible(false); 
    		...
    		// If running on a fairly small screen, just squash the search boxes.
  ...
  _selectStyle: function() {
  	// Select a style, by cycling through the available ones so each search bar looks different
  ...
  _simpleMethodSelect: function() {
    ...
    // Destroy the existing list and rebuild, with new store, display and ID fields.
  ...
  _simpleGoClick: function() {
    // record the grid configuration for this as the most recent search run by this search bar
    ...
    if ((this.simpleFor.value === undefined ||
         this.simpleFor.value === "" ) &&
        (this.inplaceAutocomplete.value === undefined ||
         this.inplaceAutocomplete.value === "")) {
  ...
  */
  _advancedGoClick: function() {
    ...
    if (this.advancedMethod.getValue() == filterPanelMode.taxonStatus ||
      ...
      this.gridConfig.distinctResultsGrids.push('taxon');
    }
    if (this.advancedMethod.getValue() == filterPanelMode.biotopeStatus ||
...
EasySDI_Map.SearchPanel = Ext.extend(Ext.Panel, {
  ...
  _getAvailableSearchTypes: function() {
    var result = [];
    // A search for Any (taxa|biotopes|any) is only enabled if there are other filter tabs present,
    // otherwise this becomes a search for anything with no filter
    var searchForAnyEnabled = componentParams.authorisedTo.ADV_SEARCH_MISC ||
        componentParams.authorisedTo.ADV_SEARCH_PLACE;
   /* var searchForAnyEnabled = componentParams.authorisedTo.ADV_SEARCH_SURVEY ||
        ...
        componentParams.authorisedTo.ADV_SEARCH_PLACE;
    if (componentParams.authorisedTo.ADV_SEARCH_TAXA) {
      result.push([filterPanelMode.taxa, EasySDI_Map.lang.getLocal('SP_OBS_SEL_TAXA')]);
      if (searchForAnyEnabled) {
        result.push([filterPanelMode.allTaxa, EasySDI_Map.lang.getLocal('SP_OBS_ANY_TAXA')]);
      }
    }
    if (componentParams.authorisedTo.ADV_SEARCH_BIOTOPE) {
      result.push([filterPanelMode.biotopes, EasySDI_Map.lang.getLocal('SP_OBS_SEL_BIOTOPES')]);
      if (searchForAnyEnabled) {
        result.push([filterPanelMode.allBiotopes, EasySDI_Map.lang.getLocal('SP_OBS_ANY_BIOTOPES')]);
      }
    }
    if (componentParams.authorisedTo.ADV_SEARCH_TAX_STS) {
      result.push([filterPanelMode.taxonStatus, EasySDI_Map.lang.getLocal('SP_OBS_TAXA_STATUS')]);
    }
    if (componentParams.authorisedTo.ADV_SEARCH_BIO_STS) {
      result.push([filterPanelMode.biotopeStatus, EasySDI_Map.lang.getLocal('SP_OBS_BIOTOPES_STATUS')]);
    }    
    if (componentParams.authorisedTo.ADV_SEARCH_TAXA &&
        componentParams.authorisedTo.ADV_SEARCH_BIOTOPE && searchForAnyEnabled) {
      result.push([filterPanelMode.all, EasySDI_Map.lang.getLocal('SP_OBS_ANYTHING')]);
    }*/
    return result;
  },

  /**
   * Add a new search bar into the panel and return it.
   * Idx is the position of the search bar in the list, defaults to the end.
   */
  ...
  _getFilter: function(innerSearchBar, searchConfig) {
    ...
    // on format of FID: that is is the featuretype followed by the id, separated
    // by a full stop.
    var filters=[];
          ...
          if (record.data.ipa_fullid == innerSearchBar.inplaceAutocomplete.value) {
            ...
            var featureType = level1[0];
              ...
              if(componentParams.autocompleteUseFID){
                // In this case the ipa_fullid is comprised of: "<featureType>:<feature_type>.<id>"
                var level2 = level1[1].split('.');
                id = level2[1];
              } else {
                // In this case the ipa_fullid is comprised of: <featureType>:<id>
                id = level1[1];
                }
                filters.push(new OpenLayers.Filter.Comparison({
