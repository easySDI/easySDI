...
var l=null;switch(layer.type.toUpperCase()){case'WMTS':extraOptions.name=layer.name;extraOptions.url=layer.url;extraOptions.layer=layer.layers;extraOptions.matrixSet=layer.matrixSet;extraOptions.matrixIds=layer.matrixIds
extraOptions.style='default';extraOptions.format=layer.imageFormat;l=new OpenLayers.Layer.WMTS(extraOptions);l.events.register('loadend',null,this.onLayerAdded);break;case'WMS':var WMSoptions={LAYERS:layer.layers,SERVICE:layer.type,VERSION:layer.version,STYLES:'',SRS:layer.projection,FORMAT:layer.imageFormat};if(layer.cache)
...
extraOptions.style='default';extraOptions.format=layer.imageFormat;l=new OpenLayers.Layer.WMTS(extraOptions);break;}
