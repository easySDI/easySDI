...
options.resolutions=layer.resolutions;if(layer.minResolution!=undefined)
options.minResolution=layer.minResolution;if(layer.maxResolution!=undefined)
options.maxResolution=layer.maxResolution;if(layer.resolutions==undefined&&layer.minResolution==undefined&&layer.maxResolution==undefined){options.minResolution="auto";options.maxResolution="auto";}
var l=null;switch(layer.type.toUpperCase()){case'WMTS':options.name=layer.name;options.url=layer.url;options.layer=layer.layers;options.matrixSet=layer.matrixSet;options.matrixIds=layer.matrixIds
options.style=layer.style;options.format=layer.imageFormat;l=new OpenLayers.Layer.WMTS(options);break;case'WMS':var WMSoptions={LAYERS:layer.layers,SERVICE:layer.url_type,VERSION:layer.version,STYLES:'',SRS:layer.projection,FORMAT:layer.imageFormat};if(layer.cache)
WMSoptions.CACHE=true;l=new OpenLayers.Layer.WMS(layer.name,layer.url,WMSoptions,options);break;};l.setVisibility(this.mapDescriber.basemapVisible);thisMap.addLayer(l);}},_addLayers=function(layer){Ext.each(this.mapDescriber.layers,function(layerToAdd){if(layerToAdd.name==layer.layers){var l='';switch(layer.url_type.toUpperCase()){case'WMS':var options={isBaseLayer:false,singleTile:layer.singletile,buffer:0,opacity:layer.defaultOpacity}
if(layer.units!=undefined)
options.units=layer.units;if(layer.projection!=undefined)
options.projection=layer.projection;if(layer.maxExtent!=undefined)
options.maxExtent=layer.maxExtent;else if(thisMap.maxExtent!=undefined)
...
break;case'WMTS':var extraOptions={isBaseLayer:false,buffer:0,opacity:layer.defaultOpacity}
