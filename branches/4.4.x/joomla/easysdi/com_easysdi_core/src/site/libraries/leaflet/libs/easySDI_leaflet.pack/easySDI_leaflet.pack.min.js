...
return _this;}
easyLegend=function(map,layertree,serviceconnector,params){var options={"zoomOnExtends":"Zoom on layer","order":"Order","download":"Download","metadata":"Info","remove":"Remove","openLegend":"Open","legendTitle":"Legend"};jQuery.extend(options,params);var _this={map:map,layertree:layertree,};var container=null;var isset=function(variable){return typeof(variable)!="undefined"&&variable!==null;};function debounce(func,wait,immediate){var timeout;return function(){var context=this,args=arguments;var later=function(){timeout=null;if(!immediate)func.apply(context,args);};var callNow=immediate&&!timeout;clearTimeout(timeout);timeout=setTimeout(later,wait);if(callNow)func.apply(context,args);};};map.on('layeradd',function(){_this.update();}).on('layerremove',function(){_this.update();}).on('zoomend',function(){_this.update();});window.addEventListener('getCapabilities',function(e){_this.update();});_this.addTo=function(div){container=div;container.on('click','.removeLayer',function(event){event.preventDefault();layertree.switchLayer(jQuery(this).data('layerid'),'off');});container.on('click','.zoomlink',function(event){event.preventDefault();var bb=jQuery(this).data('bbox').split(',');bb=L.latLngBounds([[bb[1],bb[0]],[bb[3],bb[2]]]);map.fitBounds(bb);return false;});}
