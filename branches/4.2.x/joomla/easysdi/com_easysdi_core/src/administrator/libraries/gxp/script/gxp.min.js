...
if(rec.get("filter").toString()!==filter.toString()){rec.set("filter",filter);}}else if(filter!=value){rec.set("filter",value);}},this);store.on("update",this.storeToValues,this);}});Ext.reg('gxp_classbreakslider',gxp.slider.ClassBreakSlider);Ext.namespace("gxp.slider");gxp.slider.RangeSliderTip=Ext.extend(gxp.slider.Tip,{template:'<div>{startDate}/{endDate}</div>',compiledTemplate:null,init:function(slider){this.slider=slider;this.compiledTemplate=new Ext.Template(this.template);gxp.slider.RangeSliderTip.superclass.init.call(this,slider);},getText:function(thumb){var data={startDate:this.slider.startDate,endDate:this.slider.endDate};return this.compiledTemplate.apply(data);}});Ext.namespace("gxp");Ext.override(Ext.Tip,{showBy:function(el,pos,offsets){if(Ext.isEmpty(pos)){pos=this.defaultAlign;}
...
return config;}});Ext.preg(gxp.plugins.Playback.prototype.ptype,gxp.plugins.Playback);