...
class Indicator{
    ...
    public function getData($organism,$timestart,$timeend,$limit=0){
        ...
        $query = $db->getQuery(true)
                ->select('ju.name as prod_name, count(odif.id) as count')
        ...
        $return->period_title = JText::_('COM_EASYSDI_DASHBOARD_PERIOD_TITLE_FROM')
                                ...
                                .' '.date("Y-m-d",$timeend);
        $return->columns_title = [JText::_('COM_EASYSDI_DASHBOARD_SHOP_IND_TOPUSERS_COL1'),JText::_('COM_EASYSDI_DASHBOARD_SHOP_IND_TOPUSERS_COL2')];
