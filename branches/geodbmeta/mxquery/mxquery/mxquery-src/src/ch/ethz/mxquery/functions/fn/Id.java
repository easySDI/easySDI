...
//			throw new DynamicException(ErrorCodes.A0002_EC_NOT_SUPPORTED,"To use the fn:id(), please switch to updateable variables (for now)",loc);
		...
		if (docStart.getEventType() != Type.START_DOCUMENT)
			throw new DynamicException(ErrorCodes.F0013_NO_CONTEXT_DOCUMENT, "fn:Id() only works with documents",loc);
		wnd.reset();
		current = new DescendantOrSelfIterator(context,ids,CheckNodeType.CHECK_IDTYPE_ID,loc);
	...
	public TypeInfo getStaticType() {
		return new TypeInfo(Type.START_TAG,Type.OCCURRENCE_IND_ZERO_OR_MORE,null,null);
	}	
