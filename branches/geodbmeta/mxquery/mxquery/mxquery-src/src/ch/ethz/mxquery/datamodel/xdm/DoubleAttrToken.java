...
public class DoubleAttrToken extends NamedToken {
	private final MXQueryDouble value;

	public DoubleAttrToken(Identifier id, MXQueryDouble value, QName name, XDMScope scope) throws MXQueryException{
		super(Type.createAttributeType(Type.DOUBLE), id, name, scope);
		this.value = value;
	}
	
	// currently xs:float & xs:decimal types use xs:double implementation
	public DoubleAttrToken(int type, Identifier id, MXQueryDouble value, QName name, XDMScope scope) throws MXQueryException{
		super(Type.createAttributeType(type), id, name, scope);
		this.value = value;
	}
	
	public DoubleAttrToken(DoubleAttrToken token) {
		super(token);
	...
	public NamedToken copy(QName newName) throws MXQueryException{
		return new DoubleAttrToken(this.id, this.value, newName, dynamicScope);
	}
	
	public Token copy() {
		return new DoubleAttrToken(this);
	}
}
