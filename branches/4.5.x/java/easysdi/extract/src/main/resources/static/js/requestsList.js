...
function _getRequestsTableColumnsConfiguration() {
    return [
        ...
        },
        {
            ...
            "render" : {
                "_" : function(data) {
                    return "<div><a href=\"#\" class=\"request-link\"><b>" + data.orderLabel + "</b></a></div><div>"
...
function _updateConnectorsState(connectorsInfo) {
    ...
    for (var itemIndex = 0; itemIndex < connectorsInfo.length; itemIndex++) {
        ...
        var connectorItem = $('<a class="connector-state" data-toggle="tooltip"></a>');
        connectorItem.addClass((itemData.inError) ? 'connector-state-error' : 'connector-state-success');
        connectorItem.attr('title', itemData.stateMessage);
        connectorItem.attr('href', (itemData.url) ? itemData.url : '#');

        if (itemData.inError) {
            connectorItem.append('<i class="fa fa-exclamation-triangle text-danger"></i>&nbsp;');
        }

        connectorItem.append('<i class="fa fa-plug"></i>&nbsp;');
        var textSpan = $('<span></span>');
        textSpan.text(itemData.name);
        connectorItem.append(textSpan);
        _connectorsDiv.append(connectorItem);
        connectorItem.tooltip();
    }
}
