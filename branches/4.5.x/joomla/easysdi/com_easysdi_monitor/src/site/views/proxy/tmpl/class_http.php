...
class http {
    ...
    */
	...
	$this->log .= "getFromUrl() called with url:".$url;
	...
	preg_match("~([a-z]*://)?([^:^/]*)(:([0-9]{1,5}))?(/.*)?~i", $url, $parts);
        ...
        foreach ($this->headers as $key=>$value) {
        		$this->log .=  "Request header:".$key.": ".$value." \n";
        ...
        if (count($this->postvars) > 0) {
        	$this->log .= "post_string: ".$post_string;
