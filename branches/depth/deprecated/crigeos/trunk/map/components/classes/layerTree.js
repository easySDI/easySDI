...
+'    </UserStyle>'+'  </NamedLayer>'+'</StyledLayerDescriptor>',this.layer.params.LAYERS,this.fillColor,this.opacity,this.strokeColor);}});EasySDI_Map.LayerTree=Ext.extend(Ext.data.Tree,{loadEvents:[],_loadMask:null,loadStarted:false,constructor:function(config){EasySDI_Map.LayerTree.superclass.constructor.apply(this,arguments);this._layerStore=new GeoExt.data.LayerStore({map:config.map.map});this._rootNode=new Ext.tree.TreeNode({id:'root'});this._baseLayers=new Ext.tree.TreeNode({id:'baseLayers',text:EasySDI_Map.lang.getLocal('LT_BASE_LAYERS'),singleClickExpand:true,iconCls:'folderLayer'});this._overlays=[];this._addOverlayGroups();this._rebuildTree();this.isFirstLoad=true;this._layerStore.map.events.register('moveend',this,this._onMapMoveEnd);this._layerStore.map.events.register("changelayer",this,this.onPreAddLayer);this._layerStore.map.events.register("movestart",this,this.onMoveStarted);this._layerStore.map.events.register("loadstart",this,this.onMoveStarted)
this._layerStore.map.events.register("moveend",this,this.onMoveEnd);this._loadMask=this._layerStore.map.loadingPanel;this._loadMask.activate();},onPreAddLayer:function(evt){if(evt.layer.visibility){this._loadMask.maximizeControl();this.loadEvents.push(evt.type);}else{this.onMoveEnd(evt)}},onMoveStarted:function(evt){this.loadEvents.push(evt.type);this._loadMask.maximizeControl();},onMoveEnd:function(evt){a=this.loadEvents.pop();if(this.loadEvents.length==0)
this._loadMask.hide();},_onMapMoveEnd:function(){Ext.each(SData.overlayGroups,function(group){var nodes=this._overlays['overlay_'+group.id].childNodes;Ext.each(nodes,function(node){this._updateNodeStyle(node);},this);},this);Ext.each(SData.baseLayers,function(layer){var nodes=this._baseLayers.childNodes;Ext.each(nodes,function(node){this._updateNodeStyle(node);},this);},this);this.refreshLegend();},_updateNodeStyle:function(node){if(!node.layer)
return;if(this._layerStore.map.getScale()<node.layer.maxScale||this._layerStore.map.getScale()>node.layer.minScale){node.isInScaleRange=false;if(node.rendered!==true){node.ui.render(false);node.rendered=true;}
node.getUI().addClass('hiddenLayer');node.getUI().removeClass('metadataAvailable');var el=Ext.get(node.getUI().iconNode);if(el)
el.addClass('hiddenLayerIcon');if(node.getUI().getTextEl()){if(node.getUI().getTextEl().setAttributeNS){Ext.each(SData.overlayLayers,function(layer){if(layer.name==node.layer.name){node.getUI().getTextEl().setAttributeNS("ext","qtip",EasySDI_Map.lang.getLocal('LT_SCALE_RAGE_TOOLTIP')+" "+layer.minScale+" - "+layer.maxScale);}},this);}else{Ext.each(SData.overlayLayers,function(layer){if(layer.name==node.layer.name){node.getUI().getTextEl().setAttribute("ext:qtip",EasySDI_Map.lang.getLocal('LT_SCALE_RAGE_TOOLTIP')+" "+layer.minScale+" - "+layer.maxScale);}},this);}}}else{node.isInScaleRange=true;if(node.rendered!==true){node.ui.render(false);node.rendered=true;}
node.getUI().removeClass('hiddenLayer');console.log(node.getUI());Ext.each(SData.overlayLayers,function(layer){if(layer.name==node.layer.name){if(layer.metadataUrl.length>0){node.getUI().addClass('metadataAvailable');}}},this);var el=Ext.get(node.getUI().iconNode);if(el)
...
extraOptions.style=layer.style;extraOptions.format=layer.imageFormat;l=new OpenLayers.Layer.WMTS(extraOptions);break;case'WMS':var WMSoptions={LAYERS:layer.layers,SERVICE:layer.type,VERSION:layer.version,STYLES:'',SRS:layer.projection,FORMAT:layer.imageFormat};if(layer.cache)
...
this._layerStore.map.addLayer(l);l.events.register("loadstart",this,this.onMoveStarted);l.events.register('loadend',this,this.onMoveEnd);l.events.register('visibilitychanged',this,this.onMoveEnd);l.events.register('visibilitychanged',this,this._onMapMoveEnd);if(layer.defaultVisibility)
...
l.setVisibility(false);l.events.register("loadstart",this,this.onMoveStarted);l.events.register('loadend',this,this.onMoveEnd);l.events.register('visibilitychanged',this,this.onMoveEnd);this._layerStore.map.addLayer(l);var mstyle='';var toolTip='';var metadataUrl='';var target='';if(layer.metadataUrl.length>0){mstyle='metadataAvailable';metadataUrl=layer.metadataUrl;target='_blank';}
