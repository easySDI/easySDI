...
<form
	action="<?php echo JRoute::_('index.php?option=com_easysdi_map&view=mapcontexts'); ?>"
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_EASYSDI_MAP_MAPCONTEXTS_NAME', 'a.name', $listDirn, $listOrder); ?>
				</th>
				...
				<th width="10%"><?php echo JHtml::_('grid.sort',  'JGRID_HEADING_ORDERING', 'a.ordering', $listDirn, $listOrder); ?>
					<?php if ($canOrder && $saveOrder) :?> <?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'mapcontexts.saveorder'); ?>
					<?php endif; ?>
				</th>
				<?php } ?>
		...
		<tbody>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)) { ?>
				<td class="center"><?php echo JHtml::_('jgrid.published', $item->state, $i, 'mapcontexts.', $canChange, 'cb'); ?>
				</td>
				<?php } ?>
				<?php if (isset($this->items[0]->ordering)) { ?>
				<td class="order"><?php if ($canChange) : ?> <?php if ($saveOrder) :?>
					<?php if ($listDirn == 'asc') : ?> <span><?php echo $this->pagination->orderUpIcon($i, true, 'mapcontexts.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?>
				</span> <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'mapcontexts.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?>
				</span> <?php elseif ($listDirn == 'desc') : ?> <span><?php echo $this->pagination->orderUpIcon($i, true, 'mapcontexts.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?>
				</span> <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'mapcontexts.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?>
