...
<form
	action="<?php echo JRoute::_('index.php?option=com_easysdi_map&view=layergroups'); ?>"
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_EASYSDI_MAP_LAYERGROUPS_CREATED_BY', 'a.created_by', $listDirn, $listOrder); ?>
				</th>
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_EASYSDI_MAP_LAYERGROUPS_NAME', 'a.name', $listDirn, $listOrder); ?>
				</th>
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_EASYSDI_MAP_LAYERGROUPS_ISDEFAULTOPEN', 'a.isdefaultopen', $listDirn, $listOrder); ?>
				</th>
				...
				<th width="10%"><?php echo JHtml::_('grid.sort',  'JGRID_HEADING_ORDERING', 'a.ordering', $listDirn, $listOrder); ?>
					<?php if ($canOrder && $saveOrder) :?> <?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'layergroups.saveorder'); ?>
					<?php endif; ?>
				</th>
				<?php } ?>
		...
		<tbody>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)) { ?>
				<td class="center"><?php echo JHtml::_('jgrid.published', $item->state, $i, 'layergroups.', $canChange, 'cb'); ?>
				</td>
				<?php } ?>
				<?php if (isset($this->items[0]->ordering)) { ?>
				<td class="order"><?php if ($canChange) : ?> <?php if ($saveOrder) :?>
					<?php if ($listDirn == 'asc') : ?> <span><?php echo $this->pagination->orderUpIcon($i, true, 'layergroups.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?>
				</span> <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'layergroups.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?>
				</span> <?php elseif ($listDirn == 'desc') : ?> <span><?php echo $this->pagination->orderUpIcon($i, true, 'layergroups.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?>
				</span> <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'layergroups.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?>
