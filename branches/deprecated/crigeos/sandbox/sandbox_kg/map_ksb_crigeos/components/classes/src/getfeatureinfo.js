...
EasySDI_Map.WMSGetFeatureInfo = OpenLayers.Class(OpenLayers.Control, {
	...
	/**
	 * Property: layers {Array(<OpenLayers.Layer.WMS>)} The layers to query for
	 * feature info. If omitted, all map WMS layers with a url that matches this
	 * <url> or <layerUrl> will be considered.
	 */
	layers : null,

	/**
	 * Property: queryVisible {Boolean} If true, filter out hidden layers when
	 * searching the map for layers to query. Default is false.
	 */
	queryVisible : false,

	/**
	 * Property: url {String} The URL of the WMS service to use. If not
	 * provided, the url of the first eligible layer will be used.
	 */
	url : null,

	/**
	 * Property: layerUrls {Array(String)} Optional list of urls for layers that
	 * should be queried. This can be used when the layer url differs from the
	 * url used for making GetFeatureInfo requests (in the case of a layer using
	 * cached tiles).
	 */
	layerUrls : null,

	/**
	 * Property: infoFormat {String} The mimetype to request from the server
	 */
	infoFormat : 'text/html',

	/**
	 * Property: vendorParams {Object} Additional parameters that will be added
	 * to the request, for WMS implementations that support them. This could
	 * e.g. look like (start code) { radius: 5 } (end)
	 */
	vendorParams : {},

	/**
	 * Property: format {<OpenLayers.Format>} A format for parsing
	 * GetFeatureInfo responses. Default is
	 ...
	 * (if one is not provided in the <format> property.
	 */
	formatOptions : null,
	...
	/**
	 * Property: hoverRequest {<OpenLayers.Request>} contains the currently
	 * running hover request (if any).
	 */
	hoverRequest : null,
	 ...
	 * The event object has a *text* property with the body of the response
	 * (String), a *features* property with an array of the parsed features, an
	 * *xy* property with the position of the mouse click or hover event that
	 * triggered the request, and a *request* property with the request itself.
	 */
...
/**
 * Method: findLayers Internal method to get the layers, independent of whether
 * we are inspecting the map or using a client-provided array
 */
findLayers : function() {
...
/**
 * Method: urlMatches Test to see if the provided url matches either the control
 * <url> or one of the <layerUrls>.
 * 
 * Parameters: url - {String} The url to test.
 * 
 * Returns: {Boolean} The provided url matches the control <url> or one of the
 * <layerUrls>.
 */
 ...
 * this method.
 * 
 * Valid options: - *hover* {Boolean} true if we do the request for the hover
 * handler
 */
...
/**
 * Method: handleResponse Handler for the GetFeatureInfo response.
 * 
 * Parameters: xy - {<OpenLayers.Pixel>} The position on the map where the
 * mouse event occurred. request - {XMLHttpRequest} The request object.
 */
