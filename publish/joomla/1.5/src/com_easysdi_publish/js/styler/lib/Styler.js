...
var Styler = Ext.extend(Ext.util.Observable, {
    ...
    describeLayers: function(callback) {
	    ...
	    url: styler_proxy_url+'='+styler_host+"/wms?"+ OpenLayers.Util.getParameterString(params),
            autoLoad: true,
            listeners: {
                load: function() {
                    var config, index;
    ...
    createLayout: function() {
		...
		disabled: true,
			...
			var panel = this.getLegend();
			var layerData = this.sldManager.getLayerData(this.currentLayer);
			var text = format.write(layerData.sld);
			win = new Ext.Window({
            		    title: "SLD",
            		    layout: "fit",
            		    closeAction: "hide",
			    ...
			    bbar: [{
            		        ...
            		        disabled: false,
            		        handler: function() {
				    //get edited sld from txtarea
				    sld = format.read(win.sldArea.getValue());
				    
				    //apply sld to the current layer
				    this.sldManager.setLayerData(this.currentLayer, sld);
					...
					//clean var to reload styles from server
					this.symbolTypes[this.currentLayer.id] = null;
					layerData.style = null;
					
					//ask for reload the layer
        ...
        this.layersContainer = new Ext.Panel({
            ...
            ,bbar: [{
                ...
                disabled: false,
                handler: function() {
                    window.open("http://localhost:8080/geoserver/wms/kml?layers="+this.currentLayer.params.LAYERS);
                },
                scope: this
            }]
            */
...
*/
    ...
    setCurrentLayer: function(layer) {
        if(layer != this.currentLayer) {
            ...
            for ( var elem in userLayersList )
				    {
        ...
        this.ruleDlg = new Ext.Window({
            ...
            items: [{
                ...
                attributes: new GeoExt.data.AttributeStore({
                    url: styler_proxy_url+'='+styler_host+"/wfs?",
                    baseParams: {
                        version: "1.1.1",
    ...
    getViewButton: function() {
        return this.legendContainer.getBottomToolbar().items.get(2);
    }
});
