...
class PUBLISH_Finddata {
	...
	function Finddata($featureSourceId, $featureSourceGuid, $wpsPublish, $currentUser, $config){
		...
		$script_list = array();
		$database->setQuery( "SELECT m.publish_script_id AS value, s.publish_script_display_name as text FROM #__easysdi_publish_script_map m, #__easysdi_publish_user u, #__easysdi_publish_script s WHERE u.id = m.publish_user_id AND s.id = m.publish_script_id AND u.easysdi_user_id=".$currentPartner);
		...
		$format_list = array();
		$database->setQuery( "SELECT publish_script_name AS value, publish_script_display_name as text FROM #__easysdi_publish_script WHERE publish_script_is_public=1");
		...
		$query = "SELECT name FROM #__easysdi_publish_featuresource f, #__easysdi_community_partner p where p.partner_id=f.partner_id AND p.user_id=".$joomlaUser->id;
		...
		$database->setQuery( "SELECT publish_script_name FROM #__easysdi_publish_script where id=".$config->default_datasource_handler);
		$selectedFormat = $database->loadResult();
		
		if($isUpdate)
		{
			$database->setQuery( "SELECT name FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
			$fsName = $database->loadResult();
			
			$database->setQuery( "SELECT fieldsName FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
			$fieldsName = $database->loadResult();
			
			$database->setQuery( "SELECT projection FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
			$selectedEpsg = $database->loadResult();
			
			$database->setQuery( "SELECT formatId FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
			$selectedFormat = $database->loadResult();
		}
		else
		{
