...
public class SelfAxisIterator extends CurrentBasedIterator {
	...
	public SelfAxisIterator(Context context, TypeInfo stepData,
			QueryLocation loc) {
		super(context,loc);
	...
	throws MXQueryException {
		SelfAxisIterator par = new SelfAxisIterator(context,stepData,loc);
	...
	private boolean checkToken(Token tok) throws MXQueryException {
		...
		default:
			if (Type.isAttribute(stepData.getType())) {
				if ( !CheckNodeType.checkAttributeWithType(tok, step_uri, step_local, stepData.getTypeAnnotation(), Context.getDictionary()))
					return false;
			} else 
