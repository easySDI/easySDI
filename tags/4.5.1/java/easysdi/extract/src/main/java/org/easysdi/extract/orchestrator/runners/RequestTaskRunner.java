...
import org.easysdi.extract.email.Email;
import org.easysdi.extract.email.EmailSettings;
import org.easysdi.extract.email.TaskFailedEmail;
import org.easysdi.extract.email.TaskStandbyEmail;
import org.easysdi.extract.exceptions.SystemUserNotFoundException;
...
import org.easysdi.extract.plugins.TaskProcessorRequest;
...
public class RequestTaskRunner implements Runnable {
    ...
    public RequestTaskRunner(final Request requestToProcess, final ApplicationRepositories repositories,
        ...
        if (requestToProcess.getStatus() != Status.ONGOING) {
    ...
    private Task getNextTask() {
        final org.easysdi.extract.domain.Process process = this.request.getProcess();
    ...
    private String[] getProcessOperatorsAddresses(final int processId) {
    ...
    private void prepareRequestForExport() {
        this.request.setStatus(Status.TOEXPORT);
    ...
    private void updateRequestWithResult(final RequestHistoryRecord.Status taskResultStatus,
        ...
        switch (taskResultStatus) {
            ...
            case STANDBY:
                this.request.setStatus(Status.STANDBY);
                break;
