...
/**
 * mod_easysdi_helper Helper class
 *
 * @static
 * @package		Joomla
 * @subpackage	Menus
 * @since		1.5
 */
class mod_easysdi_menu_helper
{
	function buildXML($params)
	{
		$menu = new JMenuTreeESDY($params);
		$items = &JSite::getMenu();
		...
		$isEasySDIuser = ManagerRightESDY::isEasySDIUser($user);
		$ids = array();
		$ids[0] = true;
		$last = null;
		$unresolved = array();
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_easysdi_menu');
	...
	function render(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			...
			default :
				...
				$xml = mod_easysdi_menu_helper::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class JMenuTreeESDY extends JTree
{
	/**
	...
	function addNode(&$params, $item)
	{
		...
		$node = new JMenuNodeESDY($item->id, $item->name, $item->access, $data);
 ...
 * @since		1.5
 */
class JMenuNodeESDY extends JNode
{
	/**
	 * Node Title
	 */
