...
class HTML_ctrlpanel {
	...
	function ctrlPanelPublish($format_rows, $rowPublishConfig, $options, $config_Id, $partner_list, $layers, $use_pagination_layer, $pageNav, $filter_order, $filter_order_Dir, $search){
		...
		//$diffusionServers [] = JHTML::_('select.option','0', JText::_("EASYSDI_PUBLISH_DIFFUSOR_LIST_TITLE") );
		
				//File format for the user
		$format_list = array();
		$database->setQuery( "SELECT id AS value, publish_script_display_name as text FROM #__easysdi_publish_script WHERE publish_script_is_public=1");
...
<?php
		...
		if($userId != 0){
			$query = "SELECT id FROM #__easysdi_publish_user where easysdi_user_id=".$userId;
		...
		$database->setQuery( "SELECT role_id as value, role_name as text FROM #__easysdi_community_role where role_name='EASYSDI_GEOSERVICE_DATA_MANAGER_RIGHT' OR role_name='EASYSDI_GEOSERVICE_MANAGER_RIGHT';");
		...
		$selectedRole = array();
		$database->setQuery( "SELECT role_id AS value FROM #__easysdi_community_actor WHERE partner_id=".$currentPartner);
		//$database->setQuery( "SELECT m.role_id AS value FROM #__easysdi_publish_user_role_map m, #__easysdi_publish_user u  WHERE u.id = m.publish_user_id AND u.easysdi_user_id=".$currentPartner);
		$selectedRole = $database->loadObjectList();
		
		//load scripts
		$database->setQuery( "SELECT id as value, publish_script_name as text FROM #__easysdi_publish_script where publish_script_is_public=0;");
		$script_list = $database->loadObjectList();
		$selectedScript = array();
		$database->setQuery( "SELECT m.publish_script_id AS value FROM #__easysdi_publish_script_map m, #__easysdi_publish_user u  WHERE u.id = m.publish_user_id AND u.easysdi_user_id=".$currentPartner);
...
<?php
		...
		//load scripts
		$database->setQuery( "SELECT id as value, publish_script_name as text FROM #__easysdi_publish_script;");
		$script_list = $database->loadObjectList();
