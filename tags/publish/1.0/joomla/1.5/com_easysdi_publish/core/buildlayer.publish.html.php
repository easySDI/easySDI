...
class PUBLISH_Buildlayer {
	...
	function Buildlayer($featureSources, $layerId, $layerGuid, $wpsPublish, $currentUser, $config){
		...
		$layer = null;
		if($isUpdate){
			$database->setQuery( "SELECT * FROM #__easysdi_publish_layer where id=".$layerId);
		...
		//$database->setQuery( "SELECT ETL_type FROM #__easysdi_publish_etl where id=".$config->default_etl_id);
		...
		if($isUpdate)
		{
			$database->setQuery( "SELECT name FROM #__easysdi_publish_layer where id=".$layerId);
			$layerName = $database->loadResult();
			
			$database->setQuery( "SELECT geometry FROM #__easysdi_publish_layer where id=".$layerId);
		...
		$fs_list = array();
		$query = "SELECT id AS value, name as text FROM #__easysdi_publish_featuresource where partner_id=".$currentPartner.$swlWhere;
		...
		if($featureSourceId != ""){
			...
			//featureSourceGuid
			$database->setQuery( "SELECT featureGUID FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
			$featureSourceGuid = $database->loadResult();
		
			//aliases
			$database->setQuery( "SELECT fieldsaliases FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
		...
		$query = "SELECT name FROM #__easysdi_publish_layer l, #__easysdi_community_partner p where p.partner_id=l.partner_id AND p.user_id=".$joomlaUser->id;
		$database->setQuery($query);
	   		...
	   		//echo "<pre>";  print_r($srvFs);  echo "</pre>";
				...
				//insert fields name once, is new fs, move to layer
					if($row->fieldsName == "undefined" || $row->fieldsName == "" || $row->fieldsName == "null"){
						...
						$database->setQuery( "UPDATE #__easysdi_publish_featuresource SET fieldsaliases = '".$fName."', fieldsName = '".$fName."' where id=".$row->id);
						$res = $database->query();
					}
				$i++;
			}
