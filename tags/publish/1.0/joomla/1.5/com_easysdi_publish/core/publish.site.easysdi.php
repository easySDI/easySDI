...
class SITE_publish {
	...
	function gettingStarted($userRights, $wpsPublish, $currentUser){
		...
		$query = "SELECT COUNT(*) FROM #__easysdi_publish_layer l, #__easysdi_community_partner p where p.partner_id=l.partner_id AND p.user_id=".$joomlaUser->id;					
		$db->setQuery( $query );
    ...
    $layers=null;
		...
		if($index == 0){
				...
				$query = "SELECT COUNT(*) FROM #__easysdi_publish_featuresource f, #__easysdi_community_partner p where p.partner_id=f.partner_id AND p.user_id=".$joomlaUser->id;					
				$db->setQuery( $query );
				$total = $db->loadResult();
				$pageNav = new JPagination($total,$limitstart,$limit);
				
				//Search Feature Sources regarding the limits
				$query = "SELECT * FROM #__easysdi_publish_featuresource f, #__easysdi_community_partner p where p.partner_id=f.partner_id AND p.user_id=".$joomlaUser->id;
				$query .= " ORDER BY name";
		...
		elseif($index == 1){
				...
				$query = "SELECT * FROM #__easysdi_publish_layer l, #__easysdi_community_partner p where p.partner_id=l.partner_id AND p.user_id=".$joomlaUser->id;
				$query .= " ORDER BY name";
				$db->setQuery( $query ,$pageNav->limitstart, $pageNav->limit);	
	...
	function editFeatureSource($wpsPublish, $currentUser, $config){
		...
		$db->setQuery( "SELECT featureGUID FROM #__easysdi_publish_featuresource where id=".$featureSourceId);
	...
	function deleteFeatureSource($id){
		...
		$database->setQuery( "SELECT count(*) FROM #__easysdi_publish_featuresource f, #__easysdi_publish_layer l where l.featuresourceId=f.id AND f.id=".$id);
		$result = $database->loadResult();
		if($result == 0){
				$database->setQuery( "DELETE FROM #__easysdi_publish_featuresource where id=".$id);
				$res = $database->query();
		}
	}
	...
	function editLayer($wpsPublish, $currentUser, $config){
		...
		$db->setQuery( "SELECT layerGuid FROM #__easysdi_publish_layer where id=".$layerId);
	...
	function saveLayer(){
		...
		if($layer->name != "")
		{
			if (!$layer->store()) {	
				...
				$database->setQuery( "UPDATE #__easysdi_publish_featuresource SET fieldsaliases = '".$aliases."' where id=".$layer->featuresourceId);
				$res = $database->query();
		}
		/*TODO, the Feature Source is stored, do some clean up in temp directory. Look at $featureSource->fileList */
	...
	function deleteLayer($id){
		global $mainframe;
		$database=& JFactory::getDBO();
		$database->setQuery( "DELETE FROM #__easysdi_publish_layer where id=".$id);
		$res = $database->query();
	}
	
	function showFsStats(){
		global $mainframe;
		$db =& JFactory::getDBO();
		$joomlaUser = JFactory::getUser();
		$guid = JRequest::getVar('guid',0);
		$query = "SELECT * FROM #__easysdi_publish_featuresource f, #__easysdi_community_partner p where p.partner_id=f.partner_id AND p.user_id=".$joomlaUser->id." AND f.featureGUID='".$guid."'";
		$db->setQuery($query);
