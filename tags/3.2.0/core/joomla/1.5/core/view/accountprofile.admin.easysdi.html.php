...
class HTML_accountprofile {
	function listAccountProfile(&$rows, $page, $option,  $filter_order_Dir, $filter_order)
	{
			...
			<tr>
				...
				<td width="100px" align="right">
					...
					if ($filter_order=="ordering" and $filter_order_Dir=="asc"){
						if ($disabled){
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderupAccountProfile', '', false ); ?>
				             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderdownAccountProfile', '', false ); ?>
		            <?php
						}
						else {
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderupAccountProfile', 'Move Up', isset($rows[$i-1]) ); ?>
				             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderdownAccountProfile', 'Move Down', isset($rows[$i+1]) ); ?>
					<?php
						}		
					}
					else{ 
						if ($disabled){
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderdownAccountProfile', '', false ); ?>
				             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderupAccountProfile', '', false ); ?>
		            <?php
						}
						else {
					?>
							 <?php echo $page->orderUpIcon($i, true, 'orderdownAccountProfile', 'Move Down', isset($rows[$i-1]) ); ?>
		 		             <?php echo $page->orderDownIcon($i, count($rows)-1, true, 'orderupAccountProfile', 'Move Up', isset($rows[$i+1]) ); ?>
					<?php
						}
					}?>
...
<?php
		...
		<td colspan="8"><?php echo $page->getListFooter(); ?></td>
	  	...
	  	<input type="hidden" name="task" value="listAccountProfile" />
...
<?php
	...
	function editAccountProfile(&$row, $fieldsLength, $languages, $labels, $option)
	{
		global  $mainframe;
