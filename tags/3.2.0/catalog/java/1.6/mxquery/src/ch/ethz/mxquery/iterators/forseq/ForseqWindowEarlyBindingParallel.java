...
public final class ForseqWindowEarlyBindingParallel extends ForseqWindowIterator {

	public ForseqWindowEarlyBindingParallel(Context ctx, int windowType, QName var, TypeInfo t, XDMIterator seq, WindowVariable[] startVars, XDMIterator startExpr,  WindowVariable[] endVars, XDMIterator endExpr, boolean forceEnd, boolean onNewStart, int orderMode, QueryLocation location) throws MXQueryException {
		...
		while(!endOfStream){
			...
			if(checkStartExpr(currentPosition)){
				...
				if(windowType == TUMBLING_WINDOW){
					...
					XDMIterator end = endExpr.copy(context.getParent(), null, true, new Vector());
					end.setResettable(true);
					earlyWindow = seq.getNewEarlyParallelWindowInterface(currentPosition, startVars, startVarsHolder, end, endVars, endVarsHolder);
				}
				return earlyWindow;
			}
		}
	...
	protected XDMIterator copy(Context context, XDMIterator[] subIters, Vector nestedPredCtxStack) throws MXQueryException {
		return new ForseqWindowEarlyBindingParallel(
			context, 
			windowType, 
			var.copy(), 
			varType.copy(), 
