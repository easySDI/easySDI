...
public class Ceiling extends TokenBasedIterator {
	...
	protected void init() throws MXQueryException {
		...
		switch (type) {
			...
			case Type.INTEGER:
				...
				break;
			case Type.DOUBLE:
				double val = Math.ceil( tok.getDouble().getValue());
				currentToken = new DoubleToken(null,new MXQueryDouble(val));
				break;
			case Type.FLOAT:
				float valf = (float)Math.ceil( tok.getDouble().getValue());
			...
			case Type.DECIMAL:
				...
				break;
			default:
				throw new TypeException(ErrorCodes.E0004_TYPE_INAPPROPRIATE_TYPE, "Invalid argument for fn:ceiling", loc);
		}		
