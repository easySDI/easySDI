...
public class Local_name extends TokenBasedIterator {
	protected void init() throws MXQueryException {
		...
		if (!(Type.isNode(tok.getEventType())||tok == Token.END_SEQUENCE_TOKEN)) {
			throw new TypeException(ErrorCodes.E0004_TYPE_INAPPROPRIATE_TYPE, "local-name requires a node", loc);
		}
		
		if (tok.getName() != null) {
			currentToken = new TextToken(null, tok.getLocal());
		} else {
	...
	protected XDMIterator copy(Context context, XDMIterator[] subIters, Vector nestedPredCtxStack) throws MXQueryException {
		XDMIterator copy = new Local_name();
