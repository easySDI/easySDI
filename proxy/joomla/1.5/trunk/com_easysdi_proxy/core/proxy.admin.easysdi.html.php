...
<form name='adminForm' id='adminForm' action='index.php' method='POST'><input type='hidden' name='option' value='<?php echo $option;?>'> <input
	type='hidden' name='task' value=''>

<fieldset class="adminform"><legend><?php echo JText::_( 'EASYSDI_EASYSDI_CONFIG FILE PATH' );?></legend>
<table class="admintable">
	<tr>
		<td colspan="4"><input type='text' name='filePath' size="200" value='<?php echo $xmlConfig->proxy->configFilePath;?>'></td>
	</tr>
</table>
</fieldset>
...
<div class="icon"><a href="<?php echo $link; ?>"> <?php 
$text = JText::_( 'EASYSDI_COMPONENT CONFIGURATION' );
echo JHTML::_('image.site',  'icon-48-component.png', '/templates/'. $template .'/images/header/', NULL, NULL, $text); ?> <span><?php echo $text; ?></span></a>
</div>
</div>
...
echo JHTML::_('image.site',  'icon-48-config.png', '/templates/'. $template .'/images/header/', NULL, NULL, $text ); ?> <span><?php echo $text; ?></span></a>
</div>
</div>
...
<table>
	<tr>
		...
		<td align="right" width="100%"><?php echo JText::_("EASYSDI_FILTER");?>&nbsp; <input type="text" name="search" id="search"
			value="<?php echo $search;?>" class="inputbox" onchange="document.adminForm.submit();" /></td>
		<td nowrap="nowrap"></td>
	</tr>
</table>
	...
	foreach ($xml->config as $config) {
		...
		<tr class="row<?php echo $i%2;?>">
			...
			else
			{
				...
				type='radio' id="cb<?php echo$i ?>" name="policyId" value="<?php echo $policy['Id']; ?>"></td>
			<td><a href="#edit"
				onclick="document.getElementById('configId').value='<?php echo $configId;?>';document.getElementById('task').value='editPolicy';document.getElementById('cb<?php echo $i;?>').checked=true;document.adminForm.submit();;">
				<?php echo $policy['Id']; ?> </a></td>
			<td><?php  
			...
			?></td>
			<td><?php echo $pageNav->orderUpIcon($i,  true, 'orderuppolicy', 'Move Up'); ?> <?php echo $pageNav->orderDownIcon($i,1,  true, 'orderdownpolicy', 'Move Down' ); ?>
			</td>
		</tr>
...
<table class="admintable">
	<tr>
		<td><input <?php if (strcasecmp($thePolicy->Subjects[All],'True')==0){echo 'checked';} ?> type="checkBox" name="AllUsers[]" id="AllUsers"
			onclick="disableList('AllUsers','userNameList');disableList('AllUsers','roleNameList');"> All</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
...
<?php JHTML::_( 'behavior.modal' ); ?> <?php JHTML::_('behavior.calendar'); ?>
<fieldset class="adminform"><legend><?php echo JText::_( 'EASYSDI_AVAILIBILITY'); ?></legend>
<table class="admintable">
	<tr>
		<th><b><?php echo JText::_( 'EASYSDI_DATE TIME FORMAT'); ?> </b>: <?php echo $thePolicy->{'AvailabilityPeriod'}->Mask; ?></th>
		<input name="dateFormat" type="hidden"value"dd-mm-yyyy">
		<td></td>
	</tr>
	<tr>

		<td><b><?php echo JText::_( 'EASYSDI_FROM'); ?></b> <?php echo JHTML::_('calendar',$thePolicy->{'AvailabilityPeriod'}->From->Date, "dateFrom","dateFrom","%d-%m-%Y"); ?>
		</td>
		<td><b><?php echo JText::_( 'EASYSDI_TO'); ?></b> <?php echo JHTML::_('calendar',$thePolicy->{'AvailabilityPeriod'}->To->Date, "dateTo","dateTo","%d-%m-%Y"); ?>
		</td>
...
<fieldset class="adminform"><legend><?php echo JText::_( 'EASYSDI_CSW_SERVER'); ?> <?php echo $remoteServer->url ?> <input type="button"
	value="<?php echo JText::_( 'EASYSDI_ADD NEW PARAM');?>" onClick="addNewMetadataToExclude('nbParam<?php echo $iServer; ?>',<?php echo $iServer; ?>);"></legend>
<table class="admintable">
	<thead>
		<tr>
	...
	if ($theServer !=null && $theServer->{'Metadata'} !=null && $theServer->{'Metadata'}->{'Attributes'}!=null && $theServer->{'Metadata'}->{'Attributes'}->{'Exclude'} !=null && $theServer->{'Metadata'}->{'Attributes'}->{'Exclude'}->{'Attribute'} !=null){
		foreach ($theServer->{'Metadata'}->{'Attributes'}->{'Exclude'}->{'Attribute'} as $attributeToExclude){
			?>
		<tr>
			<td><input name="param_<?php echo $iServer;?>_<?php echo $iparam;?>" type="text" class="text_area" size="200"
				value='<?php echo $attributeToExclude; ?>'></td>
		</tr>
		<?php
		$iparam  ++;
		}
...
<table class="admintable">
	<tr>
		...
		<td><input type="text" size="100" name="serverPrefixe<?php echo $iServer; ?>" id="serverPrefixe<?php echo $iServer; ?>"
			value="<?php echo $theServer->Prefix; ?>"></td>
	</tr>
	<tr>
		<td class="key"><?php echo JText::_( 'EASYSDI_WFS_SERVER_NAMESPACE'); ?></td>
		<td><input type="text" size="100" name="serverNamespace<?php echo $iServer; ?>" id="serverNamespace<?php echo $iServer; ?>"
			value="<?php echo $theServer->Namespace; ?>"></td>
	</tr>
</table>
<br>
<table class="admintable">
	<tr>
		...
		<th><b><?php echo JText::_( 'EASYSDI_LOCAL FILTER'); ?></b> <a class="modal"
			href="./index.php?option=com_easysdi_proxy&tmpl=component&task=helpQueryWMSTemplate" rel="{handler:'iframe',size:{x:600,y:280}}"> <img
			class="helpTemplate" src="../templates/easysdi/icons/silk/help.png" alt="<?php echo JText::_("EASYSDI_GEOGRAPHIC_FILTER_QUERY_TEMPLATE") ?>" /> </a></th>
	</tr>
	...
	<tr>
		...
		<table width="100%" height="100%">
			<tr valign="top">
				<td width="15"><input onClick="activateLayer('<?php echo $iServer ; ?>','<?php echo $layer->Name; ?>')"
				<?php if( HTML_proxy::isLayerChecked($theServer,$layer)) echo 'checked';?> type="checkbox"
					id="layer@<?php echo $iServer; ?>@<?php echo $layer->Name;?>" name="layer@<?php echo $iServer; ?>@<?php echo $layer->Name;?>"
		...
		<td align="center"><input <?php if(! HTML_proxy::isLayerChecked($theServer,$layer)) {echo 'disabled';}?> type="text" size="10"
			id="scaleMin@<?php echo $iServer; ?>@<?php echo $layer->Name;?>" name="scaleMin@<?php echo $iServer; ?>@<?php echo $layer->Name;?>"
			value="<?php echo HTML_proxy::getLayerMinScale($theServer,$layer); ?>"></td>
		<td align="center"><input <?php if(! HTML_proxy::isLayerChecked($theServer,$layer)) {echo 'disabled';}?> type="text" size="10"
			id="scaleMax@<?php echo $iServer; ?>@<?php echo $layer->Name;?>" name="scaleMax@<?php echo $iServer; ?>@<?php echo $layer->Name;?>"
		...
		<td><textarea <?php if(! HTML_proxy::isLayerChecked($theServer,$layer)) {echo 'disabled';}?> rows="3" cols="60"
			id="LocalFilter@<?php echo $iServer; ?>@<?php echo $layer->Name;?>" name="LocalFilter@<?php echo $iServer; ?>@<?php echo $layer->Name;?>"> <?php $localFilter = HTML_proxy ::getLayerLocalFilter($theServer,$layer); if (!(strlen($localFilter)>	0)){} else {echo $localFilter;} ?></textarea></td>
	</tr>
	<?php }}}?>
</table>
...
<table class="admintable">
	<tr>
		...
		<th><b><?php echo JText::_( 'EASYSDI_FILTERED_ATTRIBUTES_LABEL'); ?></b> <a class="modal"
			href="./index.php?option=com_easysdi_proxy&tmpl=component&task=helpAttributeFilter" rel="{handler:'iframe',size:{x:600,y:190}}"> <img
			class="helpTemplate" src="../templates/easysdi/icons/silk/help.png" alt="<?php echo JText::_("EASYSDI_GEOGRAPHIC_FILTER_QUERY_TEMPLATE") ?>" /> </a></th>
		<th><b><?php echo JText::_( 'EASYSDI_GEOGRAPHIC_FILTER_ON_QUERY'); ?></b> <a class="modal"
			href="./index.php?option=com_easysdi_proxy&tmpl=component&task=helpQueryTemplate&filter_type=filterQuery"
			rel="{handler:'iframe',size:{x:600,y:290}}"> <img class="helpTemplate" src="../templates/easysdi/icons/silk/help.png"
			alt="<?php echo JText::_("EASYSDI_GEOGRAPHIC_FILTER_QUERY_TEMPLATE") ?>" /> </a></th>

		<th><b><?php echo JText::_( 'EASYSDI_GEOGRAPHIC_FILTER_ON_ANSWER'); ?></b> <a class="modal"
			href="./index.php?option=com_easysdi_proxy&tmpl=component&task=helpQueryTemplate&filter_type=filterAnswer"
			rel="{handler:'iframe',size:{x:600,y:320}}"> <img class="helpTemplate" src="../templates/easysdi/icons/silk/help.png"
			alt="<?php echo JText::_("EASYSDI_GEOGRAPHIC_FILTER_QUERY_TEMPLATE") ?>" /> </a></th>
	</tr>
	...
	<tr>
		...
		<table width="100%" height="100%">
			<tr valign="top">
				<td width="15"><input align="left" onClick="activateFeatureType('<?php echo $iServer; ?>','<?php echo $featureType->{'Name'};?>')"
				<?php if( HTML_proxy ::isChecked($theServer,$featureType)) echo 'checked';?> type="checkbox"
					id="featuretype@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"
					name="featuretype@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>" value="<?php echo $featureType->{'Name'}; ?>"></td>
		...
		<table>
			<tr>
				<td><?php echo JText::_( 'EASYSDI_FEATURE_TYPE_SELECT_ATTRIBUTE'); ?> <input align="left"
				...
				<?php  $attributes =  HTML_proxy::getFeatureTypeAttributesList($theServer,$featureType) ;if (strcmp($attributes,"")==0){}else{echo 'checked '; } ?>
					onClick="activateAttributeList('<?php echo $iServer; ?>','<?php echo $featureType->{'Name'}; ?>')" type="checkbox"
					id="selectAttribute@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"
					name="selectAttribute@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>" value="" /></td>
			</tr>
			<tr>
				...
				<?php  $attributes =  HTML_proxy::getFeatureTypeAttributesList($theServer,$featureType) ;if (strcmp($attributes,"")==0){echo 'disabled';}?>
					id="AttributeList@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"
					name="AttributeList@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"><?php $attributes =  HTML_proxy::getFeatureTypeAttributesList($theServer,$featureType) ;if (strcmp($attributes,"")==0){}else{echo $attributes; }?></textarea>
		...
		<td><textarea rows="4" cols="50" onChange="CheckQuery('<?php echo $iServer; ?>','<?php echo $featureType->{'Name'}; ?>')"
		<?php if( ! HTML_proxy ::isChecked($theServer,$featureType)) echo 'disabled';?>
			id="RemoteFilter@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"
			name="RemoteFilter@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"><?php $remoteFilter = HTML_proxy ::getFeatureTypeRemoteFilter($theServer,$featureType);
			if (strcmp($remoteFilter,"")!=0){echo $remoteFilter;}?></textarea></td>
		<td><textarea rows="4" cols="50" onChange="CheckQuery('<?php echo $iServer; ?>','<?php echo $featureType->{'Name'}; ?>')"
		<?php if( ! HTML_proxy ::isChecked($theServer,$featureType)) echo 'disabled';?>
			id="LocalFilter@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"
			name="LocalFilter@<?php echo $iServer; ?>@<?php echo $featureType->{'Name'}; ?>"><?php $localFilter = HTML_proxy ::getFeatureTypeLocalFilter($theServer,$featureType); 
...
<table class="adminlist">
	...
	{
		?>
<h2><?php echo JText::_(  'EASYSDI_HELP_ATTRIBUTE_FILTER_TITLE' ); ?></h2>
<h3><?php echo JText::_(  'EASYSDI_HELP_ATTRIBUTE_FILTER_ALL' ); ?></h3>
<p><?php echo JText::_(  'EASYSDI_HELP_ATTRIBUTE_FILTER_ALL_CONTENT' ); ?></p>
<h3><?php echo JText::_(  'EASYSDI_HELP_ATTRIBUTE_FILTER_SELECT' ); ?></h3>
<p><?php echo JText::_(  'EASYSDI_HELP_ATTRIBUTE_FILTER_SELECT_CONTENT' ); ?></p>
		<?php
	}
	...
	{
		?>
<h2><?php echo JText::_(  'EASYSDI_HELP_IMAGE_SIZE_TITLE' ); ?></h2>
<h3><?php echo JText::_(  'EASYSDI_HELP_IMAGE_SIZE' ); ?></h3>
<p><?php echo JText::_(  'EASYSDI_HELP_IMAGE_SIZE_CONTENT' ); ?></p>
	...
	{
		?>
<h2><?php echo JText::_(  'EASYSDI_HELP_TEMPLATE_TITLE' ); ?></h2>
<h3><?php if($filter_type == "filterQuery")
{
...
<?php
	...
	function helpQueryWMSTemplate ()
	{
		?>
<h2><?php echo JText::_(  'EASYSDI_HELP_TEMPLATE_TITLE' ); ?></h2>
<h3><?php echo JText::_(  'EASYSDI_HELP_WMS_QUERY' ); ?></h3>
