 ...
 * @source $URL:
 *         http://svn.geotools.org/geotools/tags/2.4.1/modules/library/xml/
 *         src/main/java/org/geotools/xml/SchemaFactory.java $
 * @version $Id: SchemaFactory.java 26549 2007-08-14 18:00:35Z aaime $
 */
public class SchemaFactory {
	...
	private synchronized Schema getRealInstance(URI targetNamespace) {
		...
		// r = (Schema) c.getConstructor(new Class[0]).newInstance(new
		// Object[0]);
	 ...
	 */
	public synchronized static Schema getInstance(URI targetNamespace, URI desiredSchema, Level level, String user, String password) throws SAXException {
		return getInstance().getRealInstance(targetNamespace, desiredSchema, level, user, password);
	}
	 ...
	 *             When something goes wrong
	 */
	public synchronized static Schema getInstance(URI targetNamespace, URI desiredSchema, Level level) throws SAXException {
	...
	private synchronized Schema getRealInstance(URI targetNamespace2, URI desiredSchema, Level level) throws SAXException {
		...
		if ((targetNamespace == null) || (schemas.get(targetNamespace) == null)) {
			...
			try {
				...
				// TODO error handler should be notified of a
				// connectionexception: operation timed out.
	...
	private synchronized Schema getRealInstance(URI targetNamespace2, URI desiredSchema, Level level, String user, String password) throws SAXException {
		URI targetNamespace = targetNamespace2;
		if ((targetNamespace == null) || (schemas.get(targetNamespace) == null)) {
			setParser();
			...
			if (!((Schema) schemas.get(targetNamespace)).includesURI(desiredSchema)) {
				...
				try {
					...
					try {
						parser.parse(getSecuredResource(desiredSchema.toString(), user, password), contentHandler);
	...
	protected InputSource getSecuredResource(String urlstr, String userName, String password) {
		try {
			String cookie = null;
	...
	private static class MergedSchema implements Schema {
		...
		/**
		 * Returns the implementation hints. The default implementation returns
		 * en empty map.
		 */
