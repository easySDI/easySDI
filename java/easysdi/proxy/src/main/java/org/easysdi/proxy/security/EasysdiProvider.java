...
import org.easysdi.proxy.domain.Schemas;
import org.easysdi.proxy.domain.SchemasHome;
...
public class EasysdiProvider implements AuthenticationProvider,
    ...
    @Autowired
    private SchemasHome schemasHome;
    ...
    public Authentication authenticate(Authentication authentication)
        ...
        if (user == null) {
            ...
            if (authentication.getCredentials().equals(user.getPassword())) {
                ...
                Schemas schema = schemasHome.findById(700);
                String version = schema.getVersion_id();

                DefaultArtifactVersion currentVersion = new DefaultArtifactVersion(version);
                DefaultArtifactVersion maxVersion = new DefaultArtifactVersion("3.3");

                if (currentVersion.compareTo(maxVersion) == 1) {
                    //Since Joomla 3.3
                    String plaintxtpw = authentication.getCredentials().toString();
                    String hashedpwversion = user.getPassword().replace("$2y$", "$2a$");
                    if (BCrypt.checkpw(plaintxtpw, hashedpwversion)) {
                        token = new UsernamePasswordAuthenticationToken(authentication.getPrincipal().toString(), user.getPassword(), user.getAuthorities());
                    } else {
                        if (userElement != null) {
