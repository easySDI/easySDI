 ...
 */
app.ExportToKmlControl = function(opt_options) {
    ...
    var handleExportToKml = function() {
    ...
    button.addEventListener('touchstart', handleExportToKml, false);
...
ol.inherits(app.ExportToKmlControl, ol.control.Control);
 ...
 * @param {Object=} opt_options Control options.
 */
app.ExportToDxfControl = function(opt_options) {
    ...
    var DxfGlobalHandle = 2000;

    var buildDxfPolyline = function(ring) {
        console.log(ring);
        ...
        var dxfPolygon = '  0\n' +
                'POLYLINE\n' +
                '  5\n' +
                '' + decimalToHexadecimal(DxfGlobalHandle) + '\n' +
                '  8\n' +
                '0\n' +
                ' 66\n' +
                '     1\n' +
        ...
        for (var i = 0; i < pointsCount; i++) {
            ...
            dxfPolygon += '  0\n' +
                    'VERTEX\n' +
                    '  5\n' +
                    '' + decimalToHexadecimal(DxfGlobalHandle).toUpperCase() + '\n' +
                    '  8\n' +
                    '0\n' +
                    ' 10\n' +
                    '' + vertex[0] + '\n' +
    ...
    var buildDxfTemplate = function(bbox, rings) {
        console.log(rings);

        var polygons = '';

        for (var i = 0; i < rings.length; i++) {
            polygons += buildDxfPolyline(rings[i]);
        }
    ...
    var decimalToHexadecimal = function (number) {
    ...
    var getLinearRingsFromFeatures = function(features) {
    ...
    var handleExportToDxf = function() {
        var mimeType = "image/vnd.dxf";
        var featuresSource = orderGeometryLayer.getSource();
        var bbox = featuresSource.getExtent();
        var rings = getLinearRingsFromFeatures(featuresSource.getFeatures());
        _sendTextAsDownload(buildDxfTemplate(bbox, rings), mimeType, $("#requestId").val() + ".dxf");
    };


    button.addEventListener('click', handleExportToDxf, false);
    button.addEventListener('touchstart', handleExportToDxf, false);
...
ol.inherits(app.ExportToDxfControl, ol.control.Control);
