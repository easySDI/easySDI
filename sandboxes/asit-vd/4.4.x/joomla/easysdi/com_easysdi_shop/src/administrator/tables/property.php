 ...
 */
class Easysdi_shopTableproperty extends sdiTable {
    ...
    public function bind($array, $ignore = '') {
        ...
        if (($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state', 'com_easysdi_shop.property.' . $array['id']) && $array['state'] == 1)) {
            $array['state'] = 0;
        }


        if (!JFactory::getUser()->authorise('core.admin', 'com_easysdi_shop.property.' . $array['id'])) {
            $actions = JFactory::getACL()->getActions('com_easysdi_shop', 'property');
            $default_actions = JFactory::getACL()->getAssetRules('com_easysdi_shop.property.' . $array['id'])->getData();
            $array_jaccess = array();
    ...
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_easysdi_shop.property.' . (int) $this->$k;
    }
