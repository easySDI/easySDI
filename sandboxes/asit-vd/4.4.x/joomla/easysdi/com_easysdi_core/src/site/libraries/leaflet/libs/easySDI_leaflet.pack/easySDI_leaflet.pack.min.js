...
marker_layer.removeLayer(queryRes.obj);_this.updateResults();};var showRes=function(queryRes){queryRes.obj.openPopup();map.panTo(queryRes.latlng);};_this.onclick=function(e){if(jQuery('.leaflet-zoom-box-crosshair').length==0){var layerId=container.find('.queryable_layers').val();if(isset(layerId)){_this.getFeature(layertree.getLayerById(layerId),e);}}}
_this.getFeature=function(layer,event){var loc=event.containerPoint;var url=serviceconnector.getFeatureUrl(layer.layer,map,loc);var request={loading:true,layer:layer,latlng:event.latlng,html:null};_this.query.push(request);var id=_this.query.length-1;request=_this.query[id];request.id=id;_this.updateResults();jQuery.ajax({type:"GET",url:url,success:function(data){request.loading=false;if(data!=null){var table=jQuery('<div></div>').html(data).find('table');if(table.length>0){request.html='';jQuery.each(table,function(){request.html+='<table class="featureInfo easygetfeature_table">'+jQuery(this).html()+'</table>';});}else{data=data.replace('GetFeatureInfo results:','').trim();if(data.length>0&&data.search('Search returned no results.')==-1){request.html='<pre class="featureInfo easygetfeature_pre">'+data+'</pre>';}else{request.html='none';setTimeout(function(){removeRes(_this.query[request.id]);},1000);}}}
var evt=new CustomEvent('getFeature',request);window.dispatchEvent(evt);_this.updateResults();}}).fail(function(){request.html=false;_this.updateResults();});}
...
queryable.push(layer);}}}
if(queryable.length>0){container_info.append(jQuery('<p>'+options.queryablelayers_title+'</p>'));var select=jQuery('<select class="queryable_layers"></select>');container_info.append(select);jQuery.each(queryable,function(i,rqueryable){var layerId=L.Util.stamp(rqueryable.layer);jQuery('<option value="'+layerId+'"'+(current_layerID==layerId?' selected':'')+'>'+queryable[i].name+'</option>').appendTo(select);});if(jQuery('#sidebar #getfeature').hasClass('active')){if(!map.hasLayer(marker_layer))
map.addLayer(marker_layer);jQuery(map._container).addClass('getFeatureOn');}else{if(map.hasLayer(marker_layer))
map.removeLayer(marker_layer);}}else{container_info.html('<p class="warning">'+options.noqueryablelayers+'</p>');}}},250);var addQueryObj=function(query){var html_result=query.html;var nlayer=new L.Marker(query.latlng).bindPopup(html_result,{className:'easygetfeature_popup'}).addTo(marker_layer).openPopup();if(!jQuery('#sidebar #getfeature').hasClass('active')){var popup=L.popup({className:'easygetfeature_popup'}).setLatLng(query.latlng).setContent(html_result).openOn(map);}
query.obj=nlayer;}
...
var layerorder=1000;var reversegroup="";jQuery.each(contextMapData.groups,function(g,group){var overlay=(group.isbackground!='1');if(isset(group.layers)){if(group.isbackground==1){reversegroup=group.layers;}else{reversegroup=group.layers.reverse();}
...
controlLayer.addBaseLayer(layer,name);lastBaseLayer=layer;};_easySDImap.addBaseLayer=addBaseLayer;var addOverlay=function(layer,group,name){if(!isset(layer.data))layer.data={};overlays[name]=layer;if(isset(controlLayer))
...
if(options.getfeatureinfo!==false){jQuery('<li><a href="#getfeature" role="tab" title="'+i18n.t('tools_tooltips.getfeatureinfo')+'"><i class="fa fa-map-marker"></i></a></li>').appendTo(sidebar_html.find('.sidebar-tabs-top'));_this.panelFeature=jQuery('<div class="sidebar-pane" id="getfeature"></div>').appendTo(sidebar_html.find('.sidebar-content'));_this.easyGetFeature=easyGetFeature(map,controlLayer,serviceConnector,options);controlGetFeature=_this.easyGetFeature;controlGetFeature.addTo(_this.panelFeature);map.on('click',function(e){});pushTool('getfeatureinfo',controlGetFeature);}
