...
class sdiIndicatorShop_topproviders extends sdiIndicator {
    ...
    protected function _getIndicatorFileName() {
        return JText::_("COM_EASYSDI_DASHBOARD_SHOP_IND_TOPPROVIDERS_FILENAME");
    }
    ...
    protected function _getData($organism, $timestart, $timeend, $limit = 0) {
        ...
        $return->title = JText::_('COM_EASYSDI_DASHBOARD_SHOP_IND_TOPPROVIDERS_TITLE');
        $return->columns_title = array(
            JText::_('COM_EASYSDI_DASHBOARD_SHOP_IND_TOPPROVIDERS_COL1'),
            JText::_('COM_EASYSDI_DASHBOARD_SHOP_IND_TOPPROVIDERS_COL2'),
            JText::_('COM_EASYSDI_DASHBOARD_SHOP_IND_TOPPROVIDERS_COL3')
        );
    ...
    private function getBaseQuery($organism, $timestart, $timeend, $isCount = false) {
        ...
        if ($isCount) {
            ...
            $query->select('org.name as prod_name, COUNT(odif.id) as cnt, CONCAT(ROUND(count(odif.id) / ' . $this->total . ' * 100,1),\'%\') ');
        }
        ...
        if (!$isCount) {
            $query->group($db->quoteName('org.id'))
                    ->group($db->quoteName('org.name'))
