...
_this.getFeature=function(layer,event){var loc=event.containerPoint;var url=serviceconnector.getFeatureUrl(layer.layer,map,loc);request={loading:true,layer:layer,latlng:event.latlng,};_this.query.push(request);var id=_this.query.length-1;request=_this.query[id];request.id=id;request.html="";jQuery.ajax({type:"GET",url:url,success:function(data){request.loading=false;if(data!=null){var table=jQuery('<div></div>').html(data).find('table');if(table.length>0){jQuery.each(table,function(){request.html='<table class="featureInfo easygetfeature_table">'+jQuery(this).html()+'</table>';});var collapse='<div class="panel panel-default">'+'<div class="panel-heading" role="tab" id="headingOne">'+'<h4 class="panel-title">'+'<a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse'+layer.layer._leaflet_id+'" aria-expanded="true" aria-controls="collapse'+layer.layer._leaflet_id+'">'+
layer.name+'</a>'+'</h4>'+'</div>'+'<div id="collapse'+layer.layer._leaflet_id+'" class="panel-collapse collapse '+first_layer+'" role="tabpanel" aria-labelledby="heading'+layer.layer._leaflet_id+'">'+'<div class="panel-body">'+'<pre class="featureInfo easygetfeature_pre">'+request.html+'</pre>'+'</div>'+'</div>'+'</div>';tmp_collapse+=collapse;request.html=collapse_start+tmp_collapse+collapse_end;first_layer="";var evt=new CustomEvent('getFeature',request);window.dispatchEvent(evt);nbr_active_layer--;_this.updateResults();}else{data=data.replace('GetFeatureInfo results:','').trim();if(data.length>0&&data.search('Search returned no results.')==-1&&data.search('ul')>0){var collapse='<div class="panel panel-default">'+'<div class="panel-heading" role="tab" id="headingOne">'+'<h4 class="panel-title">'+'<a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse'+layer.layer._leaflet_id+'" aria-expanded="true" aria-controls="collapse'+layer.layer._leaflet_id+'">'+
...
data+'</div>'+'</div>'+'</div>';tmp_collapse+=collapse;request.html=collapse_start+tmp_collapse+collapse_end;first_layer="";var evt=new CustomEvent('getFeature',request);window.dispatchEvent(evt);_this.updateResults();}else{}}}}}).fail(function(){request.html=false;});}
...
return false;};var _getFeatureUrl=function(capabilities,layer,map,loc){var wmsParams={request:'GetFeatureInfo',query_layers:layer.data.layername,layers:layer.data.layername,info_format:'',feature_count:10,X:Math.round(loc.x),Y:Math.round(loc.y)};jQuery.each(capabilities.Capability.Request.GetFeatureInfo.Format,function(k,format){if(format=='text/plain'&&wmsParams.info_format=='')wmsParams.info_format=format;if(format=='text/html')wmsParams.info_format=format;});var bounds=map.getBounds();var size=map.getSize();var wmsVersion=parseFloat(layer.wmsParams.version);var crs=map.options.crs||map.options.crs;var projectionKey=wmsVersion>=1.3?'crs':'srs';var nw=crs.project(bounds.getNorthWest());var se=crs.project(bounds.getSouthEast());var params={'width':size.x,'height':size.y};params[projectionKey]=crs.code;params.bbox=(wmsVersion>=1.3&&crs===L.CRS.EPSG4326?[se.y,nw.x,nw.y,se.x]:[nw.x,se.y,se.x,nw.y]).join(',');L.extend(wmsParams,params);var url=capabilities.Capability.Request.GetFeatureInfo.DCPType[0].HTTP.Get.OnlineResource;url+=L.Util.getParamString(wmsParams,url);if(isset(_this.proxy))url=proxy+encodeURIComponent(url);return url;}
