...
_this.show=function(container){current_container=container;jQuery('<div class="service_switcher">'+'<label>'+options.selectserver+'</label>'+'<select class="services"></select>'+'</div>'+'<div class="available_layers"><ul></ul></div>').appendTo(container);setServicesAvailableSelect();container.on('change','.service_switcher select',function(){current_service=jQuery(this).val();setLayerAvailableSelect();});container.on('click','.available_layers a',function(){var layer=jQuery(this).data('layer');addLayer(current_service,layer);});container.on('submit','.addservice',function(e){e.preventDefault();var url=jQuery(this).find('input[name=url]').val();var type=jQuery(this).find('select[name=type]').val();var data={serviceconnector:type,servicetype:'physical',serviceurl:url,servicealias:url};current_service=url;var ns=serviceConnector.addService(data);ns.list=true;setLayerAvailableSelect();});}
return _this;}
easyLegend=function(map,layertree,serviceconnector,params){var options={"zoomOnExtends":"Zoom on layer","order":"Order","download":"Download","metadata":"Info","remove":"Remove","openLegend":"Open","legendTitle":"Legend"};jQuery.extend(options,params);var _this={map:map,layertree:layertree,};var container=null;var isset=function(variable){return typeof(variable)!="undefined"&&variable!==null;};function debounce(func,wait,immediate){var timeout;return function(){var context=this,args=arguments;var later=function(){timeout=null;if(!immediate)func.apply(context,args);};var callNow=immediate&&!timeout;clearTimeout(timeout);timeout=setTimeout(later,wait);if(callNow)func.apply(context,args);};};map.on('layeradd',function(){_this.update();}).on('layerremove',function(){_this.update();}).on('zoomend',function(){_this.update();});window.addEventListener('getCapabilities',function(e){_this.update();});_this.addTo=function(div){container=div;container.on('click','.removeLayer',function(){layertree.switchLayer(jQuery(this).data('layerid'),'off');});container.on('click','.zoomlink',function(event){event.preventDefault();var bb=jQuery(this).data('bbox').split(',');bb=L.latLngBounds([[bb[1],bb[0]],[bb[3],bb[2]]]);map.fitBounds(bb);return false;});}
...
window.addEventListener('getCapabilities',function(e){_this.update();});_this.addTo=function(div){container=div;container_info=jQuery('<div class="getfeature_info"></div>').appendTo(container);container_results=jQuery('<div class="getfeature_results"></div>').appendTo(container);container_info.on('change','select.queryable_layers',function(){current_layerID=jQuery(this).val();});container_results.on('click','.removeResult',function(e){e.stopPropagation();queryid=jQuery(this).data('queryid');removeRes(_this.query[queryid]);});container_results.on('click','.removeAllResult',function(e){e.stopPropagation();for(var i in _this.query)
removeRes(_this.query[i]);});container_results.on('click','.found',function(){queryid=jQuery(this).data('queryid');showRes(_this.query[queryid]);});}
var removeRes=function(queryRes){queryRes.html=false;if(isset(queryRes.obj))
marker_layer.removeLayer(queryRes.obj);_this.updateResults();};var showRes=function(queryRes){queryRes.obj.openPopup();map.panTo(queryRes.latlng);};_this.onclick=function(e){console.log(jQuery('.leaflet-zoom-box-crosshair').length);if(jQuery('.leaflet-zoom-box-crosshair').length==0){var layerId=container.find('.queryable_layers').val();_this.getFeature(layertree.getLayerById(layerId),e);}}
...
var addBaseLayer=function(layer,name){if(isset(layer.setZIndex)){layer.setZIndex(1);}
baseLayers[name]=layer;if(isset(controlLayer))
controlLayer.addBaseLayer(layer,name);lastBaseLayer=layer;};var addOverlay=function(layer,group,name){overlays[name]=layer;if(isset(controlLayer))
controlLayer.addOverlay(layer,name,group);};var changeIGNkey=function(url,key){if(isset(key))
