 ...
 * Easysdi_core model.
 */
class Easysdi_coreModelVersionForm extends JModelForm
{
	...
	public function &getData($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
                ...
                $id = $table->id;
                if($id){
	$canEdit = $user->authorise('core.edit', 'com_easysdi_core.version.'.$id) || $user->authorise('core.create', 'com_easysdi_core.version.'.$id);
}
else{
	$canEdit = $user->authorise('core.edit', 'com_easysdi_core') || $user->authorise('core.create', 'com_easysdi_core');
}
                if (!$canEdit && $user->authorise('core.edit.own', 'com_easysdi_core.version.'.$id)) {
	...
	{
		// Get the form.
		$form = $this->loadForm('com_easysdi_core.version', 'versionform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('version.id');
        if(JFactory::getUser()->authorise('core.delete', 'com_easysdi_core.version.'.$id) !== true){
            JError::raiseError(403, JText::_('JERROR_ALERTNOAUTHOR'));
            return false;
        }
        $table = $this->getTable();
