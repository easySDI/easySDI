...
public class JobsCollectionController extends AbstractMonitorController {
     ...
     * Creates a new job.
     *
     * @param request the request that asked for the job's creation
     * @param response the response to the request
     * @return an object containing the new job's information and which view
     * must be used to display it
     * @throws MonitorInterfaceException <ul>
     * <li>one of the job's parameters has an invalid value</li>
     * <li>a lower-level error prevented saving the new job</li>
     * </ul>
     */
     ...
     *
     * @param request the request that asked for displaying the jobs collection
     * @param response the response to the request
     * @return an object containing the public jobs collection and which view
     * must be used to display it
     * @throws MonitorInterfaceException <ul>
     * <li>The user has insufficient rights to view the private jobs collection
     * </li>
     * <li>An internal error occurred while the job list was retrieved</li>
     * </ul>
     */
    ...
    public ModelAndView show(HttpServletRequest request,
        ...
        final Boolean automatic = BooleanUtil.parseBooleanStringWithNull(
                requestParams.get("isAutomatic"));

        final Boolean realTime = BooleanUtil.parseBooleanStringWithNull(
                requestParams.get("allowsRealTime"));

        final Boolean alert = BooleanUtil.parseBooleanStringWithNull(
        ...
        try {
            ...
            start = Integer.valueOf(Integer.parseInt(requestParams.get("start")));
            limit = Integer.valueOf(Integer.parseInt(requestParams.get("limit")));
          
            sortField = requestParams.get("sort");
            direction = requestParams.get("dir");
        } catch (NumberFormatException e) {
            // TODO Auto-generated catch block
            System.out.println("start: " + requestParams.get("start"));
            System.out.println("limit: " + requestParams.get("limit"));
            start = null;
            limit = null;
            sortField = "";
            direction = "";
