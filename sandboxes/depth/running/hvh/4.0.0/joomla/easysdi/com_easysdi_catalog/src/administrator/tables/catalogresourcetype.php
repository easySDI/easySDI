 ...
 */
class Easysdi_catalogTableresourcetype extends JTable {
    ...
    public function bind($array, $ignore = '') {
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_easysdi_catalog.relationprofile.'.$array['id']) && $array['state'] == 1)){
			$array['state'] = 0;
		}

        if(!JFactory::getUser()->authorise('core.admin', 'com_easysdi_catalog.relationprofile.'.$array['id'])){
            $actions = JFactory::getACL()->getActions('com_easysdi_catalog','relationprofile');
            $default_actions = JFactory::getACL()->getAssetRules('com_easysdi_catalog.relationprofile.'.$array['id'])->getData();
            $array_jaccess = array();
