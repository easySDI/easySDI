 ...
 */
class Easysdi_catalogModelrelation extends JModelAdmin
{
	/**
	...
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			$array = array();
			foreach((array)$data->attributechild_id as $value): 
				if(!is_array($value)):
					$array[] = $value;
				endif;
			endforeach;
			$data->attributechild_id = implode(',',$array);

			//Support for multiple or not foreign key field: classchild_id
			$array = array();
			foreach((array)$data->classchild_id as $value): 
				if(!is_array($value)):
					$array[] = $value;
				endif;
			endforeach;
			$data->classchild_id = implode(',',$array);
		}
