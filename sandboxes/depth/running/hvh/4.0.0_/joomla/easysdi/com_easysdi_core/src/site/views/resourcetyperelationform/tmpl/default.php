...
<script type="text/javascript">
    ...
    getScript('//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js',function() {
        ...
        js(document).ready(function(){
            js('#form-resourcetyperelation').submit(function(event){
					...
					js('input:hidden.child_id').each(function(){
						var name = js(this).attr('name');
						if(name.indexOf('child_idhidden')){
							js('#jform_child_id option[value="'+js(this).val()+'"]').attr('selected',true);
						}
					});
					js("#jform_child_id").trigger("liszt:updated");
        });
    });
    
</script>

<div class="resourcetyperelation-edit front-end-edit">
    ...
    <form id="form-resourcetyperelation" action="<?php echo JRoute::_('index.php?option=com_easysdi_core&task=resourcetyperelation.save'); ?>" method="post" class="form-validate" enctype="multipart/form-data">
        <ul>
			...
			<div class="control-group">
				<?php $canState = false; ?>
				<?php if($this->item->id): ?>
					<?php $canState = $canState = JFactory::getUser()->authorise('core.edit.state','com_easysdi_core.resourcetyperelation'); ?>
				<?php else: ?>
					<?php $canState = JFactory::getUser()->authorise('core.edit.state','com_easysdi_core.resourcetyperelation.'.$this->item->id); ?>
			...
			<?php
				foreach((array)$this->item->parent_id as $value): 
					if(!is_array($value)):
						echo '<input type="hidden" name="jform[parent_idhidden]['.$value.']" value="'.$value.'" />';
					endif;
				endforeach;
			?>			<div class="control-group">
			...
			<?php
				foreach((array)$this->item->child_id as $value): 
					if(!is_array($value)):
						echo '<input type="hidden" name="jform[child_idhidden]['.$value.']" value="'.$value.'" />';
					endif;
				endforeach;
			?>			<div class="control-group">
			...
			<div class="control-group">
                ...
                <script type="text/javascript">
                    ...
                    jQuery('.tab-pane select').each(function(){
                       ...
                       document.getElementById("form-resourcetyperelation").appendChild(input);
        ...
        <div>
            ...
            <?php echo JText::_('or'); ?>
            <a href="<?php echo JRoute::_('index.php?option=com_easysdi_core&task=resourcetyperelation.cancel'); ?>" title="<?php echo JText::_('JCANCEL'); ?>"><?php echo JText::_('JCANCEL'); ?></a>

            <input type="hidden" name="option" value="com_easysdi_core" />
            <input type="hidden" name="task" value="resourcetyperelationform.save" />
