...
function getSynchronisationInfo(element, metadata_id, version_id){
    ...
    js.get(baseUrl + 'option=com_easysdi_core&task=version.getChildren&parentId=' + version_id, function(data) {
        var response = js.parseJSON(data);
        if (response.success == 'true') {
            if (response.num > 0) {
...
function showPublishModal(id, version_id, publishDate){
    js.get(baseUrl + 'option=com_easysdi_core&task=version.getCascadeChild&version_id=' + version_id, function(data) {
        var response = js.parseJSON(data);
