...
Ext.namespace("EasySDI_Mon");Ext.onReady(function(){Ext.QuickTips.init();var editor=new Ext.ux.grid.RowEditor({saveText:EasySDI_Mon.lang.getLocal('grid action update'),cancelText:EasySDI_Mon.lang.getLocal('grid action cancel'),clicksToEdit:2,listeners:{afteredit:syncStore}});EasySDI_Mon.MonitorRoot="index.php?option=com_easysdi_monitor";EasySDI_Mon.ExportTypeStore=[['CSV'],['XML'],['XHTML']];var expportConfig=Ext.data.Record.create(['id','exportName','exportType','exportDesc','xsltUrl']);var store=new Ext.data.Store({api:{create:EasySDI_Mon.MonitorRoot+"&task=create",read:EasySDI_Mon.MonitorRoot+"&task=read",update:EasySDI_Mon.MonitorRoot+"&task=update",destroy:EasySDI_Mon.MonitorRoot+"&task=delete"},reader:new Ext.data.JsonReader({root:'rows',totalProperty:'results',idProperty:'id'},expportConfig),writer:new Ext.data.JsonWriter({writeAllFields:true}),autoLoad:false,autoSave:false});function syncStore(rowEditor,changes,r,rowIndex){store.save();var task=new Ext.util.DelayedTask(function(){_exportGrid.getBottomToolbar().doRefresh();});task.delay(2000);}
var expander=new Ext.ux.grid.RowExpander({tpl:new Ext.Template('<p>{exportDesc}</p>'),enableCaching:false});var _exportGrid=new Ext.grid.GridPanel({id:'testGrid',loadMask:true,region:'center',stripeRows:true,title:EasySDI_Mon.lang.getLocal('export type config'),store:store,sm:new Ext.grid.RowSelectionModel({singleSelect:true}),columns:[expander,{header:EasySDI_Mon.lang.getLocal('name'),dataIndex:'exportName',width:100,id:'name-col',sortable:true,editable:true,editor:{xtype:'textfield',allowBlank:false,}},{header:EasySDI_Mon.lang.getLocal('exporttype'),dataIndex:'exportType',width:100,editor:{xtype:'combo',store:new Ext.data.SimpleStore({fields:['name'],data:EasySDI_Mon.ExportTypeStore}),displayField:'name',typeAhead:true,mode:'local',triggerAction:'all',emptyText:'',selectOnFocus:true}},{header:EasySDI_Mon.lang.getLocal('desc'),dataIndex:'exportDesc',width:270,editor:{xtype:'textfield',allowBlank:false}},{header:EasySDI_Mon.lang.getLocal('xsltUrl'),dataIndex:'xsltUrl',width:270,editor:{xtype:'textfield',allowBlank:true}}],plugins:[editor,expander],tbar:[{iconCls:'icon-service-add',text:EasySDI_Mon.lang.getLocal('grid action add'),handler:onAddExportConfig},'-',{iconCls:'icon-service-rem',ref:'../removeBtn',text:EasySDI_Mon.lang.getLocal('grid action rem'),handler:onDeleteExportConfig}],bbar:new Ext.PagingToolbar({pageSize:15,store:store,displayInfo:true,displayMsg:EasySDI_Mon.lang.getLocal('paging display msg'),emptyMsg:EasySDI_Mon.lang.getLocal('paging empty msg')}),autoExpandColumn:'name-col',collapsible:true,animCollapse:false});_exportGrid.getBottomToolbar().doLoad();function onAddExportConfig(btn,ev){var u=new _exportGrid.store.recordType();var win=new Ext.Window({width:380,autoScroll:true,modal:true,title:EasySDI_Mon.lang.getLocal('new export config'),items:[new Ext.FormPanel({labelWidth:90,monitorValid:true,ref:'exportPanel',region:'center',bodyStyle:'padding:5px 5px 0',autoHeight:true,frame:true,defaults:{width:200},defaultType:'textfield',autoHeight:true,items:[{fieldLabel:'Export config name',name:'exportName',allowBlank:false,},{xtype:'combo',mode:'local',triggerAction:'all',forceSelection:true,editable:false,fieldLabel:EasySDI_Mon.lang.getLocal('export config type'),name:'exportType',displayField:'name',valueField:'name',store:new Ext.data.SimpleStore({fields:['name'],data:EasySDI_Mon.ExportTypeStore})},{fieldLabel:'xsltUrl',name:'xsltUrl',xtype:'textfield',allowBlank:true},{fieldLabel:'description',name:'exportDesc',allowBlank:false,xtype:'textfield'}],buttons:[{text:EasySDI_Mon.lang.getLocal('grid action ok'),formBind:true,handler:function(){editor.stopEditing();var fields=win.exportPanel.getForm().getFieldValues();var plop=u;u.set('exportName',fields.exportName);u.set('exportType',fields.exportType);u.set('exportDesc',fields.exportDesc);u.set('xsltUrl',fields.xsltUrl);_exportGrid.store.insert(0,u);syncStore();win.close();}},{text:EasySDI_Mon.lang.getLocal('grid action cancel'),handler:function(){win.close();}}]})]});win.show();}
function onDeleteExportConfig(btn,ev){var sm=_exportGrid.getSelectionModel(),sel=sm.getSelected();if(sm.hasSelection()){Ext.Msg.show({title:EasySDI_Mon.lang.getLocal('Remove Export Config'),buttons:Ext.MessageBox.YESNOCANCEL,msg:EasySDI_Mon.lang.getLocal('remove')+" "+sel.data.exportName+'?',fn:function(btn){if(btn=='yes'){_exportGrid.getStore().remove(sel);syncStore();}}});}}});Ext.namespace("EasySDI_Mon");Ext.onReady(function(){Ext.QuickTips.init();var url=String.format("./components/com_easysdi_monitor/libraries/ext/src/locale/ext-lang-{0}.js",EasySDI_Mon.locale);Ext.Ajax.request({url:url,success:function(response,opts){eval(response.responseText);},failure:function(){Ext.Msg.alert('Failure',EasySDI_Mon.lang.getLocal('error_lang')+' "'+EasySDI_Mon.locale+'"');},scope:this});});Ext.form.VTypes['jobnameMask']=/[a-z0-9_-]/i;var alphanummore=/^[a-zA-Z0-9_-]+$/;Ext.form.VTypes['alphanummore']=function(v)
...
{if(Ext.form.VTypes['alphanummore'](v)){if(Ext.getCmp('SlaGrid').store.getById(v)){Ext.form.VTypes['slanameText']=EasySDI_Mon.lang.getLocal('slaname already exists');return false;}
return true;}else{Ext.form.VTypes['slanameText']=EasySDI_Mon.lang.getLocal('error ressource name');return false;}
return true;}
...
_alertForm.btnUpdate.setDisabled(false);else
_alertForm.btnUpdate.setDisabled(true);});});Ext.namespace("EasySDI_Mon");Ext.onReady(function(){var periodeditmode=false;var proxySla=new Ext.data.HttpProxy({api:{read:EasySDI_Mon.proxy+'sla',create:EasySDI_Mon.proxy+'sla',update:EasySDI_Mon.proxy+'sla',destroy:EasySDI_Mon.proxy+'sla'}});var proxyHoliday=new Ext.data.HttpProxy({api:{read:EasySDI_Mon.proxy+'holidays',create:EasySDI_Mon.proxy+'holidays',update:EasySDI_Mon.proxy+'holidays',destroy:EasySDI_Mon.proxy+'holidays'}});var proxyPeriod=new Ext.data.HttpProxy({api:{read:'?',create:'?',update:'?',destroy:'?'}});var writer=new Ext.data.JsonWriter({encode:false});var slaStore=new Ext.data.JsonStore({id:'id',root:'data',restful:true,fields:['name','isExcludeWorst','isMeasureTimeToFirst'],proxy:proxySla,writer:writer});var holidayStore=new Ext.data.JsonStore({id:'id',root:'data',restful:true,fields:['name',{name:'date',type:'date',dateFormat:'Y-m-d H:i:s'}],proxy:proxyHoliday,writer:writer});var periodStore=new Ext.data.JsonStore({id:'id',root:'data',restful:true,autoSave:false,fields:['name','isMonday','isTuesday','isWednesday','isThursday','isFriday','isSaturday','isSunday','isHolidays','slaStartTime','slaEndTime','isInclude',{name:'date',type:'date',dateFormat:'Y-m-d H:i:s'}],proxy:proxyPeriod,writer:writer});var editor=new Ext.ux.grid.RowEditor({saveText:EasySDI_Mon.lang.getLocal('grid action update'),cancelText:EasySDI_Mon.lang.getLocal('grid action cancel'),clicksToEdit:2});var editorHoliday=new Ext.ux.grid.RowEditor({saveText:EasySDI_Mon.lang.getLocal('grid action update'),cancelText:EasySDI_Mon.lang.getLocal('grid action cancel'),clicksToEdit:2});var cmSla=new Ext.grid.ColumnModel([{header:EasySDI_Mon.lang.getLocal('grid header name'),dataIndex:"name",width:100,sortable:true,editable:true,editor:{xtype:'textfield',allowBlank:false,vtype:'alphanum'}},{header:EasySDI_Mon.lang.getLocal('grid header inspire'),dataIndex:"isExcludeWorst",width:100,renderer:EasySDI_Mon.TrueFalseRenderer,editor:{xtype:'checkbox'}},{header:EasySDI_Mon.lang.getLocal('grid header measureTime'),dataIndex:"isMeasureTimeToFirst",width:120,renderer:EasySDI_Mon.TrueFalseRenderer,editor:{xtype:'checkbox'}}]);var _slaGrid=new Ext.grid.GridPanel({id:'SlaGrid',loadMask:true,region:'center',plugins:[editor],stripeRows:true,tbar:[{iconCls:'icon-service-add',text:EasySDI_Mon.lang.getLocal('grid action add'),handler:onAdd},'-',{iconCls:'icon-service-rem',ref:'../removeBtn',text:EasySDI_Mon.lang.getLocal('grid action rem'),disabled:true,handler:onDelete},'-',{text:'Add/edit periods',ref:'../editPeriodBtn',disabled:true,handler:onEditPeriods},'-',{text:'Holidays',handler:onEditHolidays}],title:EasySDI_Mon.lang.getLocal('sla list'),store:slaStore,cm:cmSla,sm:new Ext.grid.RowSelectionModel({singleSelect:true,listeners:{rowselect:function(sm,row,rec){}}})});function onAdd(btn,ev){var u=new _slaGrid.store.recordType(EasySDI_Mon.DefaultSla);var win=new Ext.Window({width:450,height:300,autoScroll:true,modal:true,layout:'fit',title:EasySDI_Mon.lang.getLocal('title new sla'),items:[new Ext.FormPanel({monitorValid:true,ref:'slaPanel',region:'center',bodyStyle:'padding:5px 5px 0;',layout:'fit',padding:5,frame:false,border:false,layout:'fit',labelWidth:1,items:[{xtype:'fieldset',title:EasySDI_Mon.lang.getLocal('field slaname title'),autoHeight:true,items:[{xtype:'textfield',value:u.data['name'],name:'name',width:280,allowBlank:false,vtype:'slaname'}]},{layout:'column',border:false,frame:false,items:[{xtype:'fieldset',columnWidth:.5,title:EasySDI_Mon.lang.getLocal('field worstexclude title'),items:[{boxLabel:EasySDI_Mon.lang.getLocal('grid header excludeWorst'),fieldLabel:'',labelSeparator:'',xtype:'checkbox',name:'isExcludeWorst',checked:u.data['isExcludeWorst']}]},{xtype:'fieldset',columnWidth:.5,title:EasySDI_Mon.lang.getLocal('field slameasure title'),defaultType:'radio',border:0,items:[{name:'isMeasureTimeToFirst',boxLabel:EasySDI_Mon.lang.getLocal('sla firstbyte boxlabel'),checked:u.data['isMeasureTimeToFirst'],fieldLabel:'',labelSeparator:'',xtype:'radio'},{name:'isMeasureTimeToFirst',boxLabel:EasySDI_Mon.lang.getLocal('sla lastbyte boxlabel'),checked:true,fieldLabel:'',xtype:'radio',labelSeparator:''}]}]}],buttons:[{text:EasySDI_Mon.lang.getLocal('grid action ok'),formBind:true,handler:function(){editor.stopEditing();var fields=win.slaPanel.getForm().getFieldValues();var u=new slaStore.recordType({name:'','isExcludeWorst':'0','isMeasureTimeToFirst':'0'});u.set('name',fields.name);u.set('isExcludeWorst',fields.isExcludeWorst);if(fields.isMeasureTimeToFirst[0])
{u.set('isMeasureTimeToFirst',true);}else
{u.set('isMeasureTimeToFirst',false);}
_slaGrid.store.insert(0,u);win.close();}},{text:EasySDI_Mon.lang.getLocal('grid action cancel'),handler:function(){win.close();}}]})]});win.show();}
function onDelete(btn,ev){var rec=_slaGrid.getSelectionModel().getSelected();if(!rec){return false;}
Ext.MessageBox.confirm(EasySDI_Mon.lang.getLocal('confirm'),String.format(EasySDI_Mon.lang.getLocal('confirm suppress sla'),rec.get('name')),function(btn){if(btn=='no')
{return false;}
else
{_slaGrid.store.remove(rec);}});}
_slaGrid.getSelectionModel().on('selectionchange',function(sm){_slaGrid.removeBtn.setDisabled(sm.getCount()<1);_slaGrid.editPeriodBtn.setDisabled(sm.getCount()<1);});function onEditPeriods(btn,ev)
{var rec=_slaGrid.getSelectionModel().getSelected();if(rec)
{periodStore.proxy.api.create.url=EasySDI_Mon.proxy+'sla/'+rec.id+'/period';periodStore.proxy.api.destroy.url=EasySDI_Mon.proxy+'sla/'+rec.id+'/period';periodStore.proxy.api.update.url=EasySDI_Mon.proxy+'sla/'+rec.id+'/period';periodStore.proxy.api.read.url=EasySDI_Mon.proxy+'sla/'+rec.id+'/period';periodStore.load();}}
function onEditHolidays()
{holidayStore.load();}
function formatDate(value){return value?value.dateFormat('Y-m-d'):'';}
var cmHoliday=new Ext.grid.ColumnModel([{header:EasySDI_Mon.lang.getLocal('grid header name'),dataIndex:"name",width:100,sortable:true,editable:true,editor:{xtype:'textfield',allowBlank:true}},{header:EasySDI_Mon.lang.getLocal('grid header holidydate'),dataIndex:"date",sortable:true,width:120,renderer:formatDate,editor:{id:'holidayDatePicker',xtype:'datefield',name:'date',allowBlank:false,format:'Y-m-d'}}]);holidayStore.on('load',function()
{var _holidayGrid=new Ext.grid.GridPanel({id:'HolidayGrid',loadMask:true,height:200,region:'center',plugins:[editorHoliday],stripeRows:true,tbar:[{iconCls:'icon-service-add',text:EasySDI_Mon.lang.getLocal('grid action add'),handler:onAddHoliday},'-',{iconCls:'icon-service-rem',ref:'../removeBtnHoliday',text:EasySDI_Mon.lang.getLocal('grid action rem'),disabled:true,handler:onDeleteHoliday}],store:holidayStore,cm:cmHoliday,sm:new Ext.grid.RowSelectionModel({singleSelect:true,listeners:{rowselect:function(sm,row,rec){}}})});var newHolidayPanel=new Ext.FormPanel({id:'holidayPanel',hidden:true,monitorValid:true,region:'center',autoHeight:true,frame:true,items:[{layout:'form',items:[{fieldLabel:EasySDI_Mon.lang.getLocal('holiday textfield label name'),id:'holidayName',allowBlank:true,xtype:'textfield',name:'name',width:120},{fieldLabel:EasySDI_Mon.lang.getLocal('holiday datepicker label date'),id:'holidayDatePicker',xtype:'datefield',name:'date',width:120,format:'Y-m-d H:i:s'}],buttons:[{text:EasySDI_Mon.lang.getLocal('grid action ok'),formBind:true,handler:function(){editorHoliday.stopEditing();var fields=Ext.getCmp('holidayPanel').getForm().getFieldValues();var u=new holidayStore.recordType({name:'','date':'2000-01-01'});u.set('name',fields.name);u.set('date',fields.date);_holidayGrid.store.insert(0,u);Ext.getCmp('holidayPanel').setVisible(false);Ext.getCmp('holidayPanel').doLayout();}},{text:EasySDI_Mon.lang.getLocal('grid action cancel'),handler:function(){Ext.getCmp('holidayPanel').setVisible(false);Ext.getCmp('holidayPanel').doLayout();}}]}]});_holidayGrid.getSelectionModel().on('selectionchange',function(sm){_holidayGrid.removeBtnHoliday.setDisabled(sm.getCount()<1);});function onAddHoliday()
{Ext.getCmp('holidayPanel').setVisible(true);Ext.getCmp('holidayPanel').doLayout();}
function onDeleteHoliday()
{var rec=_holidayGrid.getSelectionModel().getSelected();if(!rec){return false;}
Ext.MessageBox.confirm(EasySDI_Mon.lang.getLocal('confirm'),String.format(EasySDI_Mon.lang.getLocal('confirm suppress holiday'),rec.get('name')),function(btn){if(btn=='no')
{return false;}
else
{_holidayGrid.store.remove(rec);}});}
var win=new Ext.Window({width:400,autoScroll:true,modal:true,shadow:false,title:EasySDI_Mon.lang.getLocal('title new holiday'),items:[_holidayGrid,newHolidayPanel]});win.show();});periodStore.on('load',function()
{var m=new _slaGrid.store.recordType(EasySDI_Mon.DefaultPeriod);var cmPeriod=new Ext.grid.ColumnModel([{dataIndex:"name",width:200,sortable:false,editable:false}]);var topPeriodPanel=new Ext.FormPanel({id:'topPeridPanel',monitorValid:true,region:'center',autoHeight:true,frame:true,items:[{xtype:'fieldset',title:EasySDI_Mon.lang.getLocal('field period title'),autoHeight:true,anchor:'95%',items:[{layout:'column',items:[{columnWidth:.75,layout:'form',items:[{xtype:'grid',id:'periodGrid',loadMast:true,height:100,hideHeaders:true,autoScroll:true,width:240,region:'left',frame:true,stripeRows:true,store:periodStore,cm:cmPeriod,sm:new Ext.grid.RowSelectionModel({singleSelect:true,listeners:{rowselect:function(sm,row,rec){var periodform=Ext.getCmp('periodmodifyPanel').getForm();periodform.setValues({name:rec.get('name')});periodform.setValues({isMonday:rec.get('isMonday')});periodform.setValues({isTuesday:rec.get('isTuesday')});periodform.setValues({isWednesday:rec.get('isWednesday')});periodform.setValues({isThursday:rec.get('isThursday')});periodform.setValues({isFriday:rec.get('isFriday')});periodform.setValues({isSaturday:rec.get('isSaturday')});periodform.setValues({isSunday:rec.get('isSunday')});periodform.setValues({isHolidays:rec.get('isHolidays')});periodform.setValues({slaStartTime:rec.get('slaStartTime')});periodform.setValues({slaEndTime:rec.get('slaEndTime')});periodform.setValues({date:rec.get('date')});Ext.getCmp('checkInclude').setValue(rec.get('isInclude'));Ext.getCmp('checkExclude').setValue(!rec.get('isInclude'));if(rec.get('date')&&rec.get('date')!="")
{Ext.getCmp('specificdate').setValue(true);}else
{Ext.getCmp('generalrule').setValue(true);}}}})}]},{columnWidth:.25,layout:'form',items:[{id:'editperiodBtn',xtype:'button',disabled:true,text:EasySDI_Mon.lang.getLocal('button period edit'),width:80,handler:function(){periodeditmode=true;Ext.getCmp('periodmodifyPanel').setVisible(true);Ext.getCmp('periodmodifyPanel').doLayout();},style:'padding:4px'},{xtype:'button',text:EasySDI_Mon.lang.getLocal('button period add'),width:80,handler:function(){Ext.getCmp('periodGrid').getSelectionModel().clearSelections();periodeditmode=false;Ext.getCmp('periodmodifyPanel').getForm().reset();Ext.getCmp('periodmodifyPanel').setVisible(true);Ext.getCmp('periodmodifyPanel').doLayout();},style:'padding:4px'},{id:'deleteperiodbtn',xtype:'button',disabled:true,text:EasySDI_Mon.lang.getLocal('button period delete'),width:80,handler:function(){var rec=Ext.getCmp('periodGrid').getSelectionModel().getSelected();if(!rec){return false;}
Ext.MessageBox.confirm(EasySDI_Mon.lang.getLocal('confirm'),String.format(EasySDI_Mon.lang.getLocal('confirm delete period'),rec.get('name')),function(btn){if(btn=='no')
{return false;}
else
{Ext.getCmp('periodmodifyPanel').getForm().reset();Ext.getCmp('periodmodifyPanel').setVisible(false);Ext.getCmp('periodmodifyPanel').doLayout();Ext.getCmp('periodGrid').store.remove(rec);Ext.getCmp('periodGrid').store.save();}});},style:'padding:4px'}]}]}]}]});var periodEditPanel=new Ext.FormPanel({id:'periodmodifyPanel',monitorValid:true,labelWidth:1,region:'center',frame:true,hidden:true,ref:'periodPanel',autoHeight:true,items:[{xtype:'fieldset',title:EasySDI_Mon.lang.getLocal('field editperiod name title'),autoHeight:true,anchor:'95%',items:[{id:'periodname',xtype:'textfield',value:m.data['name'],name:'name',width:'90%',allowBlank:false}]},{xtype:'fieldset',title:EasySDI_Mon.lang.getLocal('field editperiod include/exclude title'),items:[{layout:'column',style:'padding-left:100px',width:250,items:[{columnWidth:.5,xtype:'radio',id:'checkInclude',name:'isInclude',boxLabel:EasySDI_Mon.lang.getLocal('period isInclude boxlabel'),labelSeparator:'',checked:m.data['isInclude']},{columnWidth:.5,xtype:'radio',id:'checkExclude',name:'isInclude',boxLabel:EasySDI_Mon.lang.getLocal('period isExclude boxlabel'),labelSeparator:'',checked:!m.data['isInclude']}]}]},{xtype:'fieldset',title:EasySDI_Mon.lang.getLocal('field period date'),items:[{layout:'column',items:[{columnWidth:.5,layout:'form',items:[{id:'specificdate',xtype:'radio',name:'datetype',boxLabel:EasySDI_Mon.lang.getLocal('period specifie date boxlabel'),labelSeparator:'',checked:false,handler:function(control)
{if(control.checked)
{Ext.getCmp('periodDatePicker').setDisabled(false);}}}]},{columnWidth:.5,layout:'form',items:[{fieldLabel:'',id:'periodDatePicker',disabled:true,xtype:'datefield',name:'date',width:100,format:'Y-m-d',value:new Date()}]}]},{id:'generalrule',xtype:'radio',name:'datetype',boxLabel:EasySDI_Mon.lang.getLocal('period general rule boxlabel'),labelSeparator:'',checked:true,handler:function(control)
{if(control.checked)
{Ext.getCmp('periodDatePicker').setDisabled(true);}}},{layout:'column',style:'padding-left:40px',width:350,items:[{id:'mondayCheck',xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period monday boxlabel'),name:'isMonday',columnWidth:.2},{id:'tuesdayCheck',xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period tuesday boxlabel'),name:'isTuesday',columnWidth:.2},{xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period wednesday boxlabel'),name:'isWednesday',columnWidth:.2},{xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period thursday boxlabel'),name:'isThursday',columnWidth:.2},{xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period friday boxlabel'),name:'isFriday',columnWidth:.2}]},{layout:'column',style:'padding-left:120px',width:240,items:[{columnWidth:.5,xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period saturday boxlabel'),name:'isSaturday'},{columnWidth:.5,xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period sunday boxlabel'),name:'isSunday'}]},{xtype:'checkbox',boxLabel:EasySDI_Mon.lang.getLocal('period holiday boxlabel'),name:'isHolidays',labelStyle:'padding-left:140px'}]},{xtype:'fieldset',title:EasySDI_Mon.lang.getLocal('period timerinterval'),autoHeight:true,items:[{layout:'column',style:'padding-left:40px',width:300,items:[{columnWidth:.5,layout:'form',items:[{id:'slatimeStart',xtype:'timefield',fieldLabel:EasySDI_Mon.lang.getLocal('period slatime start'),name:'slaStartTime',minValue:'00:00:00',maxValue:'23:00:00',value:'00:00:00',increment:60,format:'H:i:s',labelStyle:'width:30px',width:80,allowBlank:false}]},{columnWidth:.5,layout:'form',items:[{id:'slatimeEnd',xtype:'timefield',fieldLabel:EasySDI_Mon.lang.getLocal('period slatime end'),name:'slaEndTime',minValue:'00:00:00',maxValue:'23:00:00',value:'00:00:00',increment:60,format:'H:i:s',labelStyle:'width:20px',width:80,allowBlank:false}]}]}]},{layout:'column',style:'padding-left:100px',width:250,items:[{columnWidth:.5,layout:'form',items:[{xtype:'button',width:70,text:EasySDI_Mon.lang.getLocal('period save btn'),formBind:true,handler:function(){var fields=Ext.getCmp('periodmodifyPanel').getForm().getFieldValues();if(fields)
{var minSlatime=fields.slaStartTime;var maxSlatime=fields.slaEndTime;if(maxSlatime==""||minSlatime=="")
{Ext.MessageBox.alert(EasySDI_Mon.lang.getLocal('error'),EasySDI_Mon.lang.getLocal('period sla timepicket'));return false;}
var dateStr="";if(fields.datetype[0])
{if(fields.date=="")
{Ext.MessageBox.alert(EasySDI_Mon.lang.getLocal('error'),EasySDI_Mon.lang.getLocal('period date empty error'));return false;}
try
{var dt=new Date();dt=fields.date;dateStr=dt.format('Y-m-d')}catch(e)
{Ext.MessageBox.alert(EasySDI_Mon.lang.getLocal('error'),EasySDI_Mon.lang.getLocal('period date format error'));return false;}}
if(periodeditmode)
{var rec=Ext.getCmp('periodGrid').getSelectionModel().getSelected();if(!rec){return false;}
rec.beginEdit();rec.set('name',fields.name);rec.set('slaStartTime',fields.slaStartTime);rec.set('slaEndTime',fields.slaEndTime);rec.set('isInclude',fields.isInclude[0]);if(fields.datetype[0])
{rec.set('isMonday','0');rec.set('isTuesday','0');rec.set('isWednesday','0');rec.set('isThursday','0');rec.set('isFriday','0');rec.set('isSaturday','0');rec.set('isSunday','0');rec.set('isHolidays','0');rec.set('date',dateStr);}else
{rec.set('isMonday',fields.isMonday);rec.set('isTuesday',fields.isTuesday);rec.set('isWednesday',fields.isWednesday);rec.set('isThursday',fields.isThursday);rec.set('isFriday',fields.isFriday);rec.set('isSaturday',fields.isSaturday);rec.set('isSunday',fields.isSunday);rec.set('isHolidays',fields.isHolidays);rec.set('date','');}
rec.endEdit();Ext.getCmp('periodGrid').store.save();}else
{var u=new periodStore.recordType({name:'','isMonday':'0','isTuesday':'0','isWednesday':'0','isThursday':'0','isFriday':'0','isSaturday':'0','isSunday':'0','isHolidays':'0','slaStartTime':'00:00:00','slaEndTime':'00:00:00','isInclude':'1','date':''})
u.set('name',fields.name);u.set('isInclude',fields.isInclude[0]);u.set('slaStartTime',fields.slaStartTime);u.set('slaEndTime',fields.slaEndTime);if(fields.datetype[0])
{u.set('date',dateStr);}else
{u.set('isMonday',fields.isMonday);u.set('isTuesday',fields.isTuesday);u.set('isWednesday',fields.isWednesday);u.set('isThursday',fields.isThursday);u.set('isFriday',fields.isFriday);u.set('isSaturday',fields.isSaturday);u.set('isSunday',fields.isSunday);u.set('isHolidays',fields.isHolidays);u.set('date','');}
Ext.getCmp('periodGrid').store.insert(0,u);Ext.getCmp('periodGrid').store.save();}
Ext.getCmp('periodmodifyPanel').setVisible(false);Ext.getCmp('periodmodifyPanel').doLayout();}}}]},{columnWidth:.5,layout:'form',items:[{xtype:'button',width:70,text:EasySDI_Mon.lang.getLocal('period cancel btn'),handler:function(){Ext.getCmp('periodmodifyPanel').setVisible(false);Ext.getCmp('periodmodifyPanel').doLayout();}}]}]}]});var btnClose=new Ext.Button({text:EasySDI_Mon.lang.getLocal('period close btn'),width:70,style:'float:right;',handler:function(){win.close();}});var win=new Ext.Window({width:400,autoScroll:true,shadow:false,modal:true,title:EasySDI_Mon.lang.getLocal('title period window'),items:[topPeriodPanel,periodEditPanel,btnClose]});Ext.getCmp('periodGrid').getSelectionModel().on('selectionchange',function(sm){Ext.getCmp('deleteperiodbtn').setDisabled(sm.getCount()<1);Ext.getCmp('editperiodBtn').setDisabled(sm.getCount()<1);if(sm.getCount()>0)
{periodeditmode=true;}});win.show();});Ext.getCmp('SlaGrid').store.on('load',function(){EasySDI_Mon.SlaUpdateEvent.fireEvent('updatedSla');});Ext.getCmp('SlaGrid').store.on('write',function(){EasySDI_Mon.SlaUpdateEvent.fireEvent('updatedSla');});slaStore.load();});Ext.namespace("EasySDI_Mon");EasySDI_Mon.chart1;EasySDI_Mon.chart2;var aStores=Array();var tickInterval;var jobRecord;EasySDI_Mon.clearCharts=function(){if(EasySDI_Mon.chart1!=null){EasySDI_Mon.chart1.destroy();EasySDI_Mon.chart1=null;}
...
aStores=Array();};Ext.onReady(function(){var myMask=new Ext.LoadMask(Ext.getBody(),{msg:EasySDI_Mon.lang.getLocal('message wait')});var jobComboStore=new Ext.data.SimpleStore({id:'jobId',fields:[{name:'name'}],data:[]});var methodComboStore=new Ext.data.JsonStore({id:'id',root:'data',restful:true,proxy:new Ext.data.HttpProxy({url:'?'}),fields:[{id:'id'},{name:'name'}],data:{'data':[{'name':'All','value':'All'}]}});var slaComboStore=new Ext.data.SimpleStore({id:'idSlaStore',fields:[{name:'name',id:'id'}],data:[]});var exportConfig=Ext.data.Record.create(['id','exportName','exportType','exportDesc','xsltUrl']);var exportTypeStore=new Ext.data.Store({id:'exportTypeStore',proxy:new Ext.data.HttpProxy({url:EasySDI_Mon.MonitorRoot+"&task=read"}),reader:new Ext.data.JsonReader({root:'rows',totalProperty:'results',idProperty:'id'},exportConfig)});exportTypeStore.load();var body=Ext.getBody();var frame=body.createChild({tag:'iframe',cls:'x-hidden',id:'iframe',name:'iframe'});var exportForm=body.createChild({tag:'form',cls:'x-hidden',id:'form',action:EasySDI_Mon.MonitorRoot+"&task=requestExportData",method:'POST',target:'iframe'});var exportReportHandler=function(){return{doLoadExportTypes:function(btn){exportTypeStore.load();if(!btn.menu){btn.menu=new Ext.menu.Menu();}
else{btn.menu.removeAll();}
exportTypeStore.each(function(r){var action=new Ext.Action({id:r.data['id'],text:r.data['exportName'],handler:exportReportHandler.requestExportData});btn.menu.add(action)});btn.showMenu();},getExportTypes:function(btn){alert("getExportTypes");},setExportTypesAsBtnItems:function(btn){alert("setExportTypesAsBtnItems");},requestExportData:function(btn){exportTypeId=btn.id;urls=mtnBtnView_click(true);dataUrls=[urls.join(',')]
var params={exportID:exportTypeId,proxyUrls:dataUrls};exportForm.dom.action=exportForm.dom.action+'&'+Ext.urlEncode(params);exportForm.dom.submit();myMask.hide();}}}();var reportMenu=new Ext.Panel({id:'ReportMenu',layout:'table',region:'center',height:50,frame:true,layoutConfig:{columns:10},border:false,defaults:{border:false,bodyStyle:'padding:5px 5px'},collapsible:false,items:[{html:EasySDI_Mon.lang.getLocal('job')+':'},{items:[{xtype:'combo',mode:'local',id:'repCbJobs',triggerAction:'all',forceSelection:true,editable:false,fieldLabel:'Job',name:'jobComboFilter',displayField:'name',valueField:'name',emptyText:EasySDI_Mon.lang.getLocal('combo select a job'),store:jobComboStore}]},{html:EasySDI_Mon.lang.getLocal('report request select')+':'},{items:[{xtype:'combo',mode:'local',id:'repCbMeth',value:'All',triggerAction:'all',forceSelection:true,editable:false,fieldLabel:'Job',name:'reqComboFilter',displayField:'name',valueField:'name',emptyText:EasySDI_Mon.lang.getLocal('combo select a method'),store:methodComboStore}]},{html:EasySDI_Mon.lang.getLocal('period')+':'},{xtype:'combo',mode:'local',id:'repCbPeriod',triggerAction:'all',forceSelection:true,editable:false,name:'repCbPeriod',emptyText:EasySDI_Mon.lang.getLocal('combo select a period'),displayField:'name',valueField:'value',value:'today',store:new Ext.data.SimpleStore({fields:['name','value'],data:EasySDI_Mon.RepPeriodStore}),width:200},{html:EasySDI_Mon.lang.getLocal('report sla')+':'},{items:[{xtype:'combo',mode:'local',id:'repCbSla',triggerAction:'all',forceSelection:true,editable:false,fieldLabel:'Sla',name:'slaComboFilter',displayField:'name',valueField:'id',emptyText:EasySDI_Mon.lang.getLocal('report combo select a sla'),store:slaComboStore}]},{items:[{id:'mtnBtnView',xtype:'button',handler:function(){mtnBtnView_click();},listeners:{click:function(){mtnBtnView_click();}},text:EasySDI_Mon.lang.getLocal('action view')}]},{items:[{id:'exportBtn',xtype:'splitbutton',text:EasySDI_Mon.lang.getLocal('export report data'),handler:exportReportHandler.doLoadExportTypes,listeners:{arrowclick:exportReportHandler.doLoadExportTypes}}]},{},{},{},{},{},{items:[new Ext.FormPanel({id:'periodeDatePanel',layout:'table',layoutConfig:{columns:2},hidden:true,items:[{id:'minDatePicker',xtype:'datefield',name:'minDatePicker',cellCls:'ReportTableCell',width:100,format:'Y-m-d'},{id:'maxDatePicker',xtype:'datefield',name:'maxDatePicker',maxValue:new Date().format('Y-m-d'),cellCls:'ReportTableCell',width:100,format:'Y-m-d'}]})]}]});Ext.getCmp('JobGrid').store.on('load',function(){refreshComboValuesFromJobStore();});Ext.getCmp('JobGrid').store.on('write',function(){refreshComboValuesFromJobStore();});EasySDI_Mon.SlaUpdateEvent.on('updatedSla',function(){refreshComboValuesFromSlaStore();});function refreshComboValuesFromJobStore(){var aRec=Ext.getCmp('JobGrid').store.getRange();jobComboStore.removeAll();for(var i=0;i<aRec.length;i++)
{var u=new jobComboStore.recordType({name:''});u.set('name',aRec[i].get('name'));jobComboStore.insert(0,u);}}
function refreshComboValuesFromSlaStore(){var sGrid=Ext.getCmp('SlaGrid');if(sGrid)
{var aRec=Ext.getCmp('SlaGrid').store.getRange();slaComboStore.removeAll();var u=new slaComboStore.recordType({name:'',id:''});for(var i=0;i<aRec.length;i++)
{u=new slaComboStore.recordType({name:'',id:''});u.set('id',aRec[i].id);u.set('name',aRec[i].get('name'));slaComboStore.insert(0,u);}
u=new slaComboStore.recordType({name:'',id:''});u.set('id','0');u.set('name','Default');slaComboStore.insert(0,u);}}
...
var aMethods=Array();if(selMet=="All"){var aRec=methodComboStore.getRange();for(var i=0;i<aRec.length;i++){aMethods.push(aRec[i].get('name'));}}else{aMethods.push(selMet);}
var useSla=Ext.getCmp('repCbSla').getValue()!=""&&Ext.getCmp('repCbSla').getValue()!="0"?true:false;var loadedStores=0;if(null!=getUrlOnly){urls=[];for(var i=0;i<aMethods.length;i++){var slaParam="";if(useSla)
{slaParam="&useSla="+Ext.getCmp('repCbSla').getValue();}
if(aMethods[i]=="All")
{urls.push(EasySDI_Mon.proxy+EasySDI_Mon.CurrentJobCollection+'/'+selJob+'/'+logRes+'?minDate='+minDate.format('Y-m-d')+'&maxDate='+maxDate.format('Y-m-d')+'&export=true'+slaParam);}else
...
return urls;}
for(var i=0;i<aMethods.length;i++){var fields=null;if(logRes=="aggLogs")
{if(useSla)
{fields=['h1Availability','inspireNbBizErrors','h1NbConnErrors','h1MeanRespTime','inspireMeanRespTime','h1NbBizErrors','inspireAvailability','inspireNbConnErrors','inspireMaxRespTime','h1MaxRespTime','inspireMinRespTime','h1MinRespTime','h1Unavailability','inspireUnavailability','h1Failure','inspireFailure','h1Untested','inspireUntested',{name:'date',type:'date',dateFormat:'Y-m-d H:i:s'}];}else
{fields=['h24Availability','slaNbBizErrors','h24NbConnErrors','h24MeanRespTime','slaMeanRespTime','h24NbBizErrors','slaAvailability','slaNbConnErrors','slaMaxRespTime','h24MaxRespTime','slaMinRespTime','h24MinRespTime','h24Unavailability','slaUnavailability','h24Failure','slaFailure','h24Untested','slaUntested',{name:'date',type:'date',dateFormat:'Y-m-d'}];}}else
{if(aMethods[i]=="All")
{fields=[{name:'time',type:'date',dateFormat:'Y-m-d H:i:s'},'message','httpCode','status','statusCode','delay','size','avCount','unavCount','fCount','otherCount','maxTime'];}else
{fields=[{name:'time',type:'date',dateFormat:'Y-m-d H:i:s'},'message','httpCode','status','statusCode','delay','size']}}
var slaParam="";var slaID=-1;if(useSla)
{slaParam="&useSla="+Ext.getCmp('repCbSla').getValue();slaID=Ext.getCmp('repCbSla').getValue();}
if(aMethods[i]=="All")
{var summeryGraph=EasySDI_Mon.lang.getLocal('graph summery');aStores[summeryGraph]=new Ext.data.JsonStore({root:'data',autoLoad:true,totalProperty:'totalCount',restful:true,proxy:new Ext.data.HttpProxy({url:EasySDI_Mon.proxy+EasySDI_Mon.CurrentJobCollection+'/'+selJob+'/'+logRes+'?minDate='+minDate.format('Y-m-d')+'&maxDate='+maxDate.format('Y-m-d')+slaParam}),fields:fields,listeners:{load:function(){loadedStores++;if(loadedStores==aMethods.length){var b=aStores;myMask.hide();onDataReady(aStores,logRes,useSla,slaID);}}}});}else
{aStores[aMethods[i]]=new Ext.data.JsonStore({root:'data',autoLoad:true,totalProperty:'totalCount',restful:true,proxy:new Ext.data.HttpProxy({url:EasySDI_Mon.proxy+EasySDI_Mon.CurrentJobCollection+'/'+selJob+'/queries/'+aMethods[i]+'/'+logRes+'?minDate='+minDate.format('Y-m-d')+'&maxDate='+maxDate.format('Y-m-d')+slaParam}),fields:fields,listeners:{load:function(){loadedStores++;if(loadedStores==aMethods.length){var b=aStores;myMask.hide();onDataReady(aStores,logRes,useSla,slaID);}}}});}}}
...
return ticks;}
function onDataReady(aStores,logRes,useSla,slaID){if(useSla)
{var slaGrid=Ext.getCmp('SlaGrid');if(slaGrid)
{var rec=slaGrid.store.getById(slaID);if(rec)
{var showInspireGraph=false;if(rec.get('isExcludeWorst')==1)
{showInspireGraph=true;}}}}
EasySDI_Mon.chart1=EasySDI_Mon.drawResponseTimeGraph('container1',aStores,logRes,tickInterval,jobRecord,useSla,showInspireGraph);if(logRes=='logs')
{EasySDI_Mon.chart2=EasySDI_Mon.drawHealthGraphRaw('container2',aStores,logRes,useSla);}else{EasySDI_Mon.chart2=EasySDI_Mon.drawHealthGraphAgg('container2',aStores,logRes,useSla,showInspireGraph);}}});Ext.namespace("EasySDI_Mon");Ext.onReady(function(){var store=new Ext.data.SimpleStore({id:'jobId',fields:[{name:'alertId'},{name:'newStatusCode'},{name:'oldStatusCode'},{name:'cause'},{name:'httpCode'},{name:'responseDelay'},{name:'isExposedToRss',type:'boolean'},{name:'jobId',type:'int'},{name:'dateTime',type:'date',dateFormat:'Y-m-d H:i:s'},{name:'content_type'}],data:[]});var jobComboStore=new Ext.data.SimpleStore({id:'jobId',fields:[{name:'name'}],data:['All']});Ext.getCmp('JobGrid').store.on('load',function(){refreshComboValuesFromJobStore();});Ext.getCmp('JobGrid').store.on('write',function(){refreshComboValuesFromJobStore();});function refreshComboValuesFromJobStore(){var aRec=Ext.getCmp('JobGrid').store.getRange();jobComboStore.removeAll();for(var i=0;i<aRec.length;i++)
{var u=new jobComboStore.recordType({name:''});u.set('name',aRec[i].get('name'));jobComboStore.insert(0,u);}
var u=new jobComboStore.recordType({name:'All'});jobComboStore.insert(0,u);}
var cm=new Ext.grid.ColumnModel([{header:EasySDI_Mon.lang.getLocal('job'),dataIndex:"jobId",width:100,renderer:function(value){return Ext.getCmp('JobGrid').store.getAt(Ext.getCmp('JobGrid').store.findExact('id',value)).get('name');}},{header:EasySDI_Mon.lang.getLocal('status'),dataIndex:"newStatusCode",width:64,renderer:function(newStatus,scope,row){return EasySDI_Mon.AlertStatusRenderer(newStatus,scope,row);}},{header:EasySDI_Mon.lang.getLocal('cause'),dataIndex:"cause",width:100},{header:EasySDI_Mon.lang.getLocal('delay'),dataIndex:"responseDelay",width:60,renderer:EasySDI_Mon.DelayRenderer},{header:EasySDI_Mon.lang.getLocal('grid header httpcode'),dataIndex:"httpCode",width:60},{header:EasySDI_Mon.lang.getLocal('grid header dateTime'),dataIndex:"dateTime",width:150,sortable:true,renderer:EasySDI_Mon.DateTimeRenderer},{header:EasySDI_Mon.lang.getLocal('grid header responseview'),dataIndex:"alertId",width:100,renderer:function(value,scope,row){var response_url=EasySDI_Mon.proxy+'image/alert/'+value+'?contenttype='+row.data.content_type;return'<a href="'+response_url+'" target="_blank">'+EasySDI_Mon.lang.getLocal('grid alert view')+'</a>';}}]);var _alertsGrid=new Ext.grid.GridPanel({id:'AlertGrid',region:'center',stripeRows:true,title:EasySDI_Mon.lang.getLocal('alert list'),loadMask:new Ext.LoadMask(Ext.getBody(),{msg:EasySDI_Mon.lang.getLocal('message wait')}),store:store,cm:cm,listeners:{headerclick:function(grid,colnum){if(grid.colModel.config[colnum].sortable==true)
this.getBottomToolbar().moveFirst();}},bbar:new Ext.PagingToolbar({ref:'../gridPag',pageSize:15,store:store,displayInfo:true,displayMsg:'Affichage alertes {0} à {1} de {2}',emptyMsg:"Aucun job à afficher"}),tbar:[{xtype:'combo',mode:'local',ref:'../cbJobs',triggerAction:'all',forceSelection:true,editable:false,fieldLabel:EasySDI_Mon.lang.getLocal('job'),name:'jobComboFilter',displayField:'name',valueField:'name',emptyText:EasySDI_Mon.lang.getLocal('combo select a job'),store:jobComboStore}]});_alertsGrid.cbJobs.on('select',function(cmb,rec){store.removeAll();if(rec.get('name')!='All'){loadAlertData(rec.get('name'),1,1,false);}else{loadAlertData(null,1,1,true);}});function loadAlertData(jobName,current,total,getAll){var myMask=new Ext.LoadMask(Ext.getCmp('AlertGrid').getEl(),{msg:EasySDI_Mon.lang.getLocal('message wait')});myMask.show();proxyUrl="";if((null==jobName)&&(getAll==true))
proxyUrl=EasySDI_Mon.proxy+EasySDI_Mon.CurrentJobCollection+'/all/alerts'+"&all=true";else if((null!=jobName)&&(getAll==false))
proxyUrl=EasySDI_Mon.proxy+EasySDI_Mon.CurrentJobCollection+'/'+jobName+'/alerts';else{console.log("No job name provided");return false;}
store.removeAll();var newStore=new Ext.data.JsonStore({root:'data',proxy:new Ext.data.HttpProxy({url:proxyUrl}),restful:true,idProperty:'id',totalProperty:'count',remoteSort:true,fields:['newStatusCode','oldStatusCode','cause','httpCode','responseDelay','isExposedToRss','jobId',{name:'dateTime',type:'date',dateFormat:'Y-m-d H:i:s'},'alertId','content_type'],sortInfo:{field:"dateTime",direction:"DESC"},listeners:{load:function(){store.removeAll();var aRec=this.getRange();for(var j=0;j<aRec.length;j++)
{store.add(aRec[j]);}}}});component=Ext.getCmp('AlertGrid');var colModel=component.getColumnModel();component.reconfigure(newStore,colModel);if(component.getBottomToolbar()){component.getBottomToolbar().show();component.getBottomToolbar().bindStore(newStore);component.getBottomToolbar().doRefresh();}
myMask.hide();}});Ext.namespace("EasySDI_Mon");Ext.onReady(function(){var editmode=true;function findMainURL(name)
...
Ext.getCmp('mtnCbJobs').on("select",function(field,newValue,oldValue){Ext.getCmp('mtnDelDaily').setDisabled(false);Ext.getCmp('mtnDelAgg').setDisabled(false);});});Ext.namespace("EasySDI_Mon");EasySDI_Mon.mainPanel=2;Ext.onReady(function(){var appPanel
var item1=new Ext.Panel({title:EasySDI_Mon.lang.getLocal('advanced'),layout:'fit',border:false,frame:true,autoScroll:true,items:[Ext.getCmp('jobAdvForm')]});var item2=new Ext.Panel({title:EasySDI_Mon.lang.getLocal('requests'),layout:'fit',border:false,frame:true,autoScroll:true,items:[Ext.getCmp('ReqGrid')]});var item3=new Ext.Panel({title:EasySDI_Mon.lang.getLocal('alerts'),layout:'fit',border:false,frame:true,autoScroll:true,items:[Ext.getCmp('AlertForm')]});var accordion=new Ext.Panel({region:'east',split:true,width:'40%',layout:'accordion',items:[item1,item2,item3]});var jobPanel={title:EasySDI_Mon.lang.getLocal('jobs'),frame:true,xtype:'panel',layout:'border',region:'center',border:false,items:[accordion,Ext.getCmp('JobGrid')]};var reportPanel={id:'reportPanel',title:EasySDI_Mon.lang.getLocal('reports'),xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('ReportMenu')]};var alertPanel={title:EasySDI_Mon.lang.getLocal('alerts'),xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('AlertGrid')]};var statePanel={title:EasySDI_Mon.lang.getLocal('state'),xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('JobStateGrid')]};var maintenancePanel={title:EasySDI_Mon.lang.getLocal('maintenance'),xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('MaintenancePanel')]};responseoverviewPanel={title:EasySDI_Mon.lang.getLocal('responseoverview'),xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('ResponseOverviewPanel')]};slaPanel={title:EasySDI_Mon.lang.getLocal('sla'),frame:true,xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('SlaGrid')]};exportPanel={title:EasySDI_Mon.lang.getLocal('export'),xtype:'panel',layout:'border',region:'center',border:false,items:[Ext.getCmp('testGrid')]};var cardTabs=new Ext.TabPanel({id:'card-tabs-panel',activeTab:EasySDI_Mon.defaultTab,defaults:{bodyStyle:'padding:15px'},items:[statePanel,jobPanel,reportPanel,alertPanel,maintenancePanel,slaPanel]});var appPanel=new Ext.Panel({id:'appPanel',frame:true,anchor:'50%',region:'center',layout:'card',margins:'2 5 5 0',activeItem:0,border:true,items:[cardTabs]});EasySDI_Mon.mainPanel=new Ext.Panel({height:EasySDI_Mon.appHeight,id:'mainPanel',xtype:'panel',renderTo:"tabsContainer",layout:'border',border:false,frame:false,items:[new Ext.Panel({region:'north',height:35,id:'JobCollectionPanel',ref:'JobCollectionPanel',border:false,frame:false,layout:'table',layoutConfig:{columns:2},items:[{html:EasySDI_Mon.lang.getLocal('job collection')+':',handleMouseEvents:false,border:false},{xtype:'combo',border:false,mode:'local',id:'jobCbCollection',triggerAction:'all',forceSelection:true,editable:false,value:EasySDI_Mon.DefaultJobCollection,name:'jobCbCollection',displayField:'name',valueField:'value',store:new Ext.data.SimpleStore({fields:['name','value'],data:EasySDI_Mon.JobCollectionStore})}]}),appPanel]});Ext.getCmp('jobCbCollection').on('select',function(cmb,rec){var store=Ext.getCmp('JobGrid').store;EasySDI_Mon.CurrentJobCollection=rec.data.value;store.proxy.setUrl(EasySDI_Mon.proxy+rec.data.value);store.proxy.api.create.url=EasySDI_Mon.proxy+rec.data.value;store.proxy.api.destroy.url=EasySDI_Mon.proxy+rec.data.value;store.proxy.api.read.url=EasySDI_Mon.proxy+rec.data.value;store.proxy.api.update.url=EasySDI_Mon.proxy+rec.data.value;store.load();});if(cardTabs.getActiveTab().id=='reportPanel')
...
{}});Ext.namespace("EasySDI_Mon");EasySDI_Mon.drawHealthGraphRaw=function(container,aStores,logRes,useSla){var options={chart:{renderTo:container,defaultSeriesType:'column'},title:{text:EasySDI_Mon.lang.getLocal('service health')},xAxis:{categories:[]},yAxis:{min:0,max:100,title:{text:EasySDI_Mon.lang.getLocal('percentage')}},legend:{backgroundColor:'#FFFFFF',reversed:true},tooltip:{formatter:function(){return''+
...
{avCount=0;unavCount=0;fCount=0;otherCount=0;if(typeof aStores[storeName]!='function'){var aRec=aStores[storeName].getRange();var summaryCount=0;for(var i=0;i<aRec.length;i++)
{if(aRec[i].get('avCount')>=0)
{avCount+=aRec[i].get('avCount');fCount+=aRec[i].get('fCount');unavCount+=aRec[i].get('unavCount');otherCount+=aRec[i].get('otherCount');summaryCount=avCount+fCount+unavCount+otherCount;}else
...
if(summaryCount>0)
{avSeries.data.push(Math.round((avCount/summaryCount)*10000)/100);unavSeries.data.push(Math.round((unavCount/summaryCount)*10000)/100);fSeries.data.push(Math.round((fCount/summaryCount)*10000)/100);otherSeries.data.push(Math.round((otherCount/summaryCount)*10000)/100);}else
{avSeries.data.push(Math.round((avCount/aRec.length)*10000)/100);unavSeries.data.push(Math.round((unavCount/aRec.length)*10000)/100);fSeries.data.push(Math.round((fCount/aRec.length)*10000)/100);otherSeries.data.push(Math.round((otherCount/aRec.length)*10000)/100);}}}
options.series.push(otherSeries);options.series.push(unavSeries);options.series.push(fSeries);options.series.push(avSeries);var chart=new Highcharts.Chart(options);return chart;};Ext.namespace("EasySDI_Mon");EasySDI_Mon.drawHealthGraphAgg=function(container,aStores,logRes,useSla,showInspireGraph){var options={chart:{renderTo:container,defaultSeriesType:'column'},title:{text:EasySDI_Mon.lang.getLocal('service health summary')},xAxis:{categories:[]},yAxis:{min:0,max:100,title:{text:EasySDI_Mon.lang.getLocal('percentage')}},legend:{backgroundColor:'#FFFFFF',reversed:true},tooltip:{formatter:function(){return''+
this.series.name+': '+this.y+'%';}},plotOptions:{series:{stacking:'normal'}},series:[]};var avSeries={name:EasySDI_Mon.lang.getLocal('available'),data:[],color:'#7dff9c'};var unavSeries={name:EasySDI_Mon.lang.getLocal('unavailable'),data:[],color:'#ff7f7f'};var fSeries={name:EasySDI_Mon.lang.getLocal('failure'),data:[],color:'#e2ff1d'};var otherSeries={name:EasySDI_Mon.lang.getLocal('untested-unknown'),data:[],color:'#b3b3b3'};for(var storeName in aStores)
{if(typeof aStores[storeName]!='function'){if(useSla)
{if(!showInspireGraph)
{options.xAxis.categories.push(storeName);}else
{options.xAxis.categories.push(storeName);}}else
{options.xAxis.categories.push(storeName+EasySDI_Mon.lang.getLocal('h24 suffix'));options.xAxis.categories.push(storeName+EasySDI_Mon.lang.getLocal('sla suffix'));}}}
var avCountH24;var avCountSLA;var unavCount;var unavCountSP;var fCount;var fCountSP
var otherCount;var otherCountSP;if(useSla)
{for(var storeName in aStores)
{if(typeof aStores[storeName]!='function'){var aRec=aStores[storeName].getRange();avCountH24=0;avCountSLA=0;unavCount=0;unavCountSP=0;fCount=0;fCountSP=0;otherCount=0;otherCountSP=0;for(var i=0;i<aRec.length;i++)
{avCountH24+=aRec[i].get('h1Availability');avCountSLA+=aRec[i].get('inspireAvailability');unavCount+=aRec[i].get('h1Unavailability');unavCountSP+=aRec[i].get('inspireUnavailability');fCount+=aRec[i].get('h1Failure');fCountSP+=aRec[i].get('inspireFailure');otherCount+=aRec[i].get('h1Untested');otherCountSP+=aRec[i].get('inspireUntested');}
if(!showInspireGraph)
{avSeries.data.push(Math.round((avCountH24/aRec.length)*100)/100);unavSeries.data.push(Math.round((unavCount/aRec.length)*100)/100);fSeries.data.push(Math.round((fCount/aRec.length)*100)/100);otherSeries.data.push(Math.round((otherCount/aRec.length)*100)/100);}else{avSeries.data.push(Math.round((avCountSLA/aRec.length)*100)/100);unavSeries.data.push(Math.round((unavCountSP/aRec.length)*100)/100);fSeries.data.push(Math.round((fCountSP/aRec.length)*100)/100);otherSeries.data.push(Math.round((otherCountSP/aRec.length)*100)/100);}}}}else
{for(var storeName in aStores)
{if(typeof aStores[storeName]!='function'){var aRec=aStores[storeName].getRange();avCountH24=0;avCountSLA=0;unavCount=0;unavCountSP=0;fCount=0;fCountSP=0;otherCount=0;otherCountSP=0;for(var i=0;i<aRec.length;i++)
{avCountH24+=aRec[i].get('h24Availability');avCountSLA+=aRec[i].get('slaAvalabilty');unavCount+=aRec[i].get('h24Unavailability');unavCountSP+=aRec[i].get('slaUnavailability');fCount+=aRec[i].get('h24Failure');fCountSP+=aRec[i].get('slaFailure');otherCount+=aRec[i].get('h24Untested');otherCountSP+=aRec[i].get('slaUntested');}
avSeries.data.push(Math.round((avCountH24/aRec.length)*100)/100);avSeries.data.push(Math.round((avCountSLA/aRec.length)*100)/100);unavSeries.data.push(Math.round((unavCount/aRec.length)*100)/100);unavSeries.data.push(Math.round((unavCountSP/aRec.length)*100)/100);fSeries.data.push(Math.round((fCount/aRec.length)*100)/100);fSeries.data.push(Math.round((fCountSP/aRec.length)*100)/100);otherSeries.data.push(Math.round((otherCount/aRec.length)*100)/100);otherSeries.data.push(Math.round((otherCountSP/aRec.length)*100)/100);}}}
options.series.push(otherSeries);options.series.push(unavSeries);options.series.push(fSeries);options.series.push(avSeries);var chart=new Highcharts.Chart(options);return chart;};Ext.namespace("EasySDI_Mon");EasySDI_Mon.drawHealthLineGraph=function(container,aStores,logRes,tickInterval){var options={chart:{renderTo:container,marginRight:130,zoomType:'x'},title:{text:EasySDI_Mon.lang.getLocal('service health'),x:-20},xAxis:{title:EasySDI_Mon.lang.getLocal('grid header dateTime'),type:'datetime',maxZoom:tickInterval/10,tickInterval:tickInterval},yAxis:{title:{text:EasySDI_Mon.lang.getLocal('percentage')}},tooltip:{formatter:function(){return'<b>'+this.series.name+'</b><br/>'+
...
{tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip AVAILABILITY')+":</b> "+(Math.round(this.point.data.data.h1Availability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNAVAILABILITY')+":</b> "+(Math.round(this.point.data.data.h1Unavailability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip FAILURE')+":</b> "+(Math.round(this.point.data.data.h1Failure*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNTESTED')+":</b> "+(Math.round(this.point.data.data.h1Untested*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_CONN_ERRORS')+":</b> "+this.point.data.data.h1NbConnErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_BIZ_ERRORS')+":</b> "+this.point.data.data.h1NbBizErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MaxRespTime')+"</b> -> "+Math.round(this.point.data.data.h1MaxRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MinRespTime')+"</b> -> "+Math.round(this.point.data.data.h1MinRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";}else
{tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip AVAILABILITY')+":</b> "+(Math.round(this.point.data.data.inspireAvailability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNAVAILABILITY')+":</b> "+(Math.round(this.point.data.data.inspireUnavailability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip FAILURE')+":</b> "+(Math.round(this.point.data.data.inspireFailure*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNTESTED')+":</b> "+(Math.round(this.point.data.data.inspireUntested*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_CONN_ERRORS')+":</b> "+this.point.data.data.inspireNbConnErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_BIZ_ERRORS')+":</b> "+this.point.data.data.inspireNbBizErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MaxRespTime')+"</b> -> "+Math.round(this.point.data.data.inspireMaxRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MinRespTime')+"</b> -> "+Math.round(this.point.data.data.inspireMinRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";}}
else
{if(this.point.normalGraph)
{tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip AVAILABILITY')+":</b> "+(Math.round(this.point.data.data.h24Availability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNAVAILABILITY')+":</b> "+(Math.round(this.point.data.data.h24Unavailability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip FAILURE')+":</b> "+(Math.round(this.point.data.data.h24Failure*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNTESTED')+":</b> "+(Math.round(this.point.data.data.h24Untested*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_CONN_ERRORS')+":</b> "+this.point.data.data.h24NbConnErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_BIZ_ERRORS')+":</b> "+this.point.data.data.h24NbBizErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MaxRespTime')+"</b> -> "+Math.round(this.point.data.data.h24MaxRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MinRespTime')+"</b> -> "+Math.round(this.point.data.data.h24MinRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";}else
{tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip AVAILABILITY')+":</b> "+(Math.round(this.point.data.data.slaAvailability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNAVAILABILITY')+":</b> "+(Math.round(this.point.data.data.slaUnavailability*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip FAILURE')+":</b> "+(Math.round(this.point.data.data.slaFailure*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip UNTESTED')+":</b> "+(Math.round(this.point.data.data.slaUntested*100)/100)+"%<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_CONN_ERRORS')+":</b> "+this.point.data.data.slaNbConnErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip NB_BIZ_ERRORS')+":</b> "+this.point.data.data.slaNbBizErrors+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MaxRespTime')+"</b> -> "+Math.round(this.point.data.data.slaMaxRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MinRespTime')+"</b> -> "+Math.round(this.point.data.data.slaMinRespTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";}}}else
{tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip response size')+"</b>: "+Math.round(this.point.data.data.size)+" bytes<br/>";if(this.point.data.data.maxTime)
{tip+="<b>"+EasySDI_Mon.lang.getLocal('tooltip MaxRespTime')+"</b>: "+Math.round(this.point.data.data.maxTime*1000)+EasySDI_Mon.lang.getLocal('ms suffix')+"<br/>";}else
...
{if(typeof aStores[storeName]!='function'){var aRec=aStores[storeName].getRange();series={data:[]};if(useSla)
{series.name=storeName;}else
{series.name=storeName+'[h24]';}
for(var i=0;i<aRec.length;i++)
{if(logRes=='aggLogs')
{var point;if(useSla)
{point={x:aRec[i].get('date').getTime(),y:Math.round(aRec[i].get('h1MeanRespTime')*1000)!=-1?Math.round(aRec[i].get('h1MeanRespTime')*1000):0,data:aRec[i],log:logRes,normalGraph:true};}else
{point={x:aRec[i].get('date').getTime(),y:Math.round(aRec[i].get('h24MeanRespTime')*1000)!=-1?Math.round(aRec[i].get('h24MeanRespTime')*1000):0,data:aRec[i],log:logRes,normalGraph:true};}
...
var point={x:aRec[i].get('time').getTime(),y:Math.round(aRec[i].get('delay')*1000)!=-1?Math.round(aRec[i].get('delay')*1000):0,marker:{fillColor:color},data:aRec[i],log:logRes,normalGraph:true};series.data.push(point);}}
if(useSla&&showInspireGraph&&logRes=='aggLogs')
{}else
{options.series.push(series);}
if(logRes=='aggLogs'){series={data:[]};if(useSla)
{series.name=storeName;}else
{series.name=storeName+EasySDI_Mon.lang.getLocal('sla suffix');}
for(var i=0;i<aRec.length;i++)
{var point;if(useSla)
{point={x:aRec[i].get('date').getTime(),y:Math.round(aRec[i].get('inspireMeanRespTime')*1000)!=-1?Math.round(aRec[i].get('inspireMeanRespTime')*1000):0,data:aRec[i],log:logRes,normalGraph:false};}else
{point={x:aRec[i].get('date').getTime(),y:Math.round(aRec[i].get('slaMeanRespTime')*1000)!=-1?Math.round(aRec[i].get('slaMeanRespTime')*1000):0,data:aRec[i],log:logRes,normalGraph:false};}
if(useSla&&!showInspireGraph)
{}else
{series.data.push(point);}}
if(useSla&&!showInspireGraph)
{}else{options.series.push(series);}}}}
var chart=new Highcharts.Chart(options);return chart;};