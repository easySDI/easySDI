...
public class WMSProxyServlet extends ProxyServlet {
    ...
    /**
     * Fill by the WMSProxyServletGetMapThread with <index of layer in the
     * request,<path,alias>>
     */
    ...
    /**
     * Fill by the WMSProxyServletGetFEatureInfoThread with <index of layer in
     * the request,<path,alias>>
     */
    ...
    /**
     * Protocol version of the remote server response. A GetCapabilities
     * response can be made in a protocol version different from the one
     * requested.
     *
    ...
    public void requestPreTreatmentGetCapabilities(HttpServletRequest req, HttpServletResponse resp) {
        try {
            ...
            for (WMSProxyServerGetCapabilitiesThread serverThreadList1 : serverThreadList) {
                serverThreadList1.join();
            }
    ...
    /**
     * Overwrite the GetCapabilities response with config and policy
     * informations : - Service metadata - Authorized operations - Authorized
     * layers - BBOX (geographic filter) - online resources Merge all the remote
     * responses into one single file to send to the client.
     *
     * @param req
     * @param resp
     */
    public void transformGetCapabilities(HttpServletRequest req, HttpServletResponse resp) {
        try {
            ...
            if ((remoteServerExceptionFiles.size() > 0 && sdiVirtualService.getSdiSysExceptionlevel().getValue().equals("restrictive"))
                    || (sdiVirtualService.getSdiSysExceptionlevel().getValue().equals("permissive") && wmsGetCapabilitiesResponseFilePathMap.isEmpty())) {
    ...
    /**
     * Apply the geographic filter, defined in the policy, on the response from
     * remote servers. Buffer image.
     *
     * @param req
     * @param resp
     */
...
//                }
    ...
    protected boolean isSizeInTheRightRange(int currentWidth, int currentHeight) {
        ...
        if (sdiPolicy.getWms_maximumheight() != null && sdiPolicy.getWms_maximumheight().length() > 0) {
            maxHeight = Integer.parseInt(sdiPolicy.getWms_maximumheight());
        }
        if (sdiPolicy.getWms_maximumwidth() != null && sdiPolicy.getWms_maximumwidth().length() > 0) {
            maxWidth = Integer.parseInt(sdiPolicy.getWms_maximumwidth());
        }
        if (sdiPolicy.getWms_minimumheight() != null && sdiPolicy.getWms_minimumheight().length() > 0) {
            minHeight = Integer.parseInt(sdiPolicy.getWms_minimumheight());
        }
        if (sdiPolicy.getWms_minimumwidth() != null && sdiPolicy.getWms_minimumwidth().length() > 0) {
            minWidth = Integer.parseInt(sdiPolicy.getWms_minimumwidth());
        }
