...
var panel=this.mapPanel;var map=panel.map;extent=record.getLayer().maxExtent.clone();var records=baseRecords.concat(overlayRecords);if(records.length){panel.layers.add(records);}}
this.checkLayerRecordQueue();};Ext.namespace("sdi.geoext.data");sdi.geoext.data.PrintProvider=Ext.extend(GeoExt.data.PrintProvider,{loadCapabilities:function(){if(!this.url){return;}
