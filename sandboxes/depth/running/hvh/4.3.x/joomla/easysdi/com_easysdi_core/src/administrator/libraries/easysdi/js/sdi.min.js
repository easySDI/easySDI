...
this.setValue(value);var layers=this.map.layers;for(var a=0;a<layers.length;a++){this.redrawLayer(layers[a]);}},changeIndoorLevelByCode:function(slider,code){if(!code)
return;levels.forEach(function(level){if(level.code==code)
slider.changeIndoorLevel(slider,levels.indexOf(level));})},redrawLayer:function(layer){var level=this.getLevel();if(layer.isindoor&&layer.isindoor==1&&layer.levelfield){var servertype=layer.servertype;if(servertype==1){layer.mergeNewParams({'CQL_FILTER':"\""+layer.levelfield+"="+level.code+"\""});}else if(servertype==2){layer.mergeNewParams({'layerDefs':"{\""+layer.params.LAYERS+"\":\""+layer.levelfield+"='"+level.code+"'\"}"});}else if(servertype==3){layer.mergeNewParams({'layerDefs':"{\""+layer.params.LAYERS+"\":\""+layers[a].levelfield+"='"+level.code+"'\"}"});layer.mergeNewParams({'CQL_FILTER':"\""+layer.levelfield+"="+level.code+"\""});}
layer.redraw(true);window.appname.mapPanel.map.events.triggerEvent("layerredrawn",{layer:layer});}},getLevel:function(value){if(!value){value=this.getValue();}
