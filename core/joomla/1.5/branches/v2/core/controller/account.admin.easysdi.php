...
class ADMIN_account {
	...
	function listAccount($option) {
		...
		// Dï¿½compte des enregistrements totaux
		if ($type == '') {
	...
	function exportAccount( $cid, $option ) {
		...
		if (!is_array( $cid ) || count( $cid ) < 1) {
			//echo "<script> alert('Sï¿½lectionnez un enregistrement ï¿½ exporter'); window.history.go(-1);</script>\n";
			$mainframe->enqueueMessage('Sï¿½lectionnez un enregistrement ï¿½ exporter','error');
			$mainframe->redirect("index.php?option=$option&task=listAccount" );
			exit;
		}
